#*****************************************************************************
#*Function: Read in raw data and merged all sites data by form - STACKED DATA
...
# LINE 39 - set upload date
# LINE 44 - set vector of all sites with data in that upload 
#*****************************************************************************
...
# set path to save 
path_to_save = paste0("~/Monitoring Report/data/stacked/" ,UploadDate, "/")
...
merged_dir <- paste0("~/Monitoring Report/data/stacked", sep = "")
...
# 
# # Jan 11 data check: 
# m00_merged <- read.csv("Z:/Stacked Data/2024-01-19/mnh00_merged.csv")
# table(m00_merged$SITE)
# # 
# m02_merged <- read.csv("Z:/Stacked Data/2024-01-19/mnh02_merged.csv")
# table(m02_merged$SITE)
# # 
# pak_enrolled <- MatData_Screen_Enroll %>% filter(SITE == "Pakistan", ENROLL == 1) %>% select(SITE,SCRNID, MOMID, PREGID, ENROLL)
# # ke_enrolled <- MatData_Screen_Enroll %>% filter(SITE == "Kenya", ENROLL == 1) %>% select(SITE,SCRNID, MOMID, PREGID, ENROLL)
# # gha_enrolled <- MatData_Screen_Enroll %>% filter(SITE == "Ghana", ENROLL == 1) %>% select(SITE,SCRNID, MOMID, PREGID, ENROLL)
# # cmc_enrolled <- MatData_Screen_Enroll %>% filter(SITE == "India-CMC", ENROLL == 1) %>% select(SITE,SCRNID, MOMID, PREGID, ENROLL)
# 
# m02_merged_pak <- m02_merged %>% filter(MOMID %in% as.vector(pak_enrolled$MOMID)) %>% select(SITE, SCRNID, MOMID, PREGID)
# m00_merged_pak <- m00_merged %>% filter(SCRNID %in% as.vector(pak_enrolled$SCRNID)) %>% select(SITE, SCRNID)
# dim(m02_merged_pak)
# dim(m00_merged_pak)
# ## Pakistan has 2452 enrolled and is missing n = 2010 pre screening and n = 0 screening
# 
# m02_merged_ke <- m02_merged %>% filter(MOMID %in% as.vector(ke_enrolled$MOMID)) %>% select(SITE, SCRNID, MOMID, PREGID)
# m00_merged_ke <- m00_merged %>% filter(SCRNID %in% as.vector(ke_enrolled$SCRNID)) %>% select(SITE, SCRNID)
# dim(m00_merged_ke)
# dim(m02_merged_ke)
# ## kenya has 1098 enrolled and is missing n = 0 pre screening and n = 0 screening
# 
# m02_merged_gha <- m02_merged %>% filter(MOMID %in% as.vector(gha_enrolled$MOMID)) %>% select(SITE, SCRNID, MOMID, PREGID)
# m00_merged_gha <- m00_merged %>% filter(SCRNID %in% as.vector(gha_enrolled$SCRNID)) %>% select(SITE, SCRNID)
# dim(m00_merged_gha)
# dim(m02_merged_gha)
# ## Ghana has 948 enrolled and is missing n = 0 pre screening and n = 0 screening
# 
# m02_merged_cmc <- m02_merged %>% filter(MOMID %in% as.vector(cmc_enrolled$MOMID)) %>% select(SITE, SCRNID, MOMID, PREGID)
# m00_merged_cmc <- m00_merged %>% filter(SCRNID %in% as.vector(cmc_enrolled$SCRNID)) %>% select(SITE, SCRNID)
# dim(m00_merged_cmc)
# dim(m02_merged_cmc)
# ## CMC has 367 enrolled and is missing n = 1 pre screening and n = 0 screening
...
# setwd(paste0("D:/Users/stacie.loisate/Documents/Monitoring Report/data/stacked/",UploadDate, "/"))
...
# 
# m02 <- read.csv("Z:/SynapseCSVs/Pakistan/2023-12-08/mnh02.csv")
# # m05 <- read.csv("Z:/Stacked Data/2023-12-08/mnh05_merged.csv")
# # 
# test <- m02 %>% 
#   filter(MOMID != "n/a") %>% 
#   group_by(MOMID) %>% 
#   mutate(N = n()) %>% 
#   select(SCRNID, MOMID, PREGID, N)
# 
# # summary by site 
# for (i in unique(m05$SITE)) {
#   df <- m05 %>% filter(SITE == i)
#   print(paste0(i, "="))
#   print(summary(df$M05_MUAC_PERES))
# }
# summary(m05$muac_peres)
# 
# ggplot(data = m05, aes(x = muac_peres)) +
#   geom_histogram()
