...
vars_criteria <- matData %>%
  ...
  mutate(
    ...
    AGE_ENROLL = case_when(
      !is.na(M02_SCRN_OBSSTDAT) & !is.na(M00_BRTHDAT) & 
        M00_BRTHDAT != "07/07/1907" & M00_BRTHDAT != "07-07-1907" ~ as.numeric(dmy(M02_SCRN_OBSSTDAT) - dmy(M00_BRTHDAT)) %/% 365,
      TRUE ~ M00_ESTIMATED_AGE
    ),
    CRIT_AGE = case_when(
      AGE_ENROLL < 18 | AGE_ENROLL > 34 ~ 0,
  ...
  mutate( 
    ...
    CRIT_PREV_PRETERM_LBW = case_when(
      ...
      M04_PRETERM_RPORRES_V1 == 0 & M04_LOWBIRTHWT_RPORRES_V1 == 0 ~ 1,
    ...
    # 14. no HIV M04_HIV_EVER_MHOCCUR_V1(1,0,99); M04_HIV_MHOCCUR_V1(1,0.99): M06_HIV_POC_LBPERF_V, M06_HIV_POC_LBORRES_V1(1.0)
    CRIT_HIV = case_when(
      M04_HIV_EVER_MHOCCUR_V1 == 1 | M04_HIV_MHOCCUR_V1 == 1 | M06_HIV_POC_LBORRES_V1 == 1 ~ 0,
      M04_HIV_EVER_MHOCCUR_V1 == 0 & M04_HIV_MHOCCUR_V1 == 0 & M06_HIV_POC_LBORRES_V1 == 0 ~ 1,
      M04_HIV_EVER_MHOCCUR_V1 == 99 | M04_HIV_MHOCCUR_V1 == 99 | M06_HIV_POC_LBPERF_V1 == 0 ~ 0
    ),
    ...
    # 20. no iron deficiency (not iron deficient: serum ferritin > 15 mcg/L) data unit is µg/dL couble check before use
    CRIT_IRON = case_when(
...
healthyOutcome <- vars_criteria %>% 
  rowwise() %>%
  mutate(HEALTHY_CHECK = sum(across(starts_with("CRIT_"), ~ .x >= 0), na.rm = TRUE)) %>% 
  mutate(
    HEALTHY_ELIGIBLE = case_when(
      if_any(starts_with("CRIT_"), ~. == 0) ~ 0, #Not eligible
