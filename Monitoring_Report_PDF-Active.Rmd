---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2024-10-22)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
**Included data last updated in Synapse:** 2024-10-18 (Zambia: 2024-10-04)
...
-   Ghana: 2024-09-13
-   Zambia: 2024-09-25
-   India CMC: 2024-10-04
-   India SAS: 2024-10-10
...
### Table 12: PNC protocol compliance for PRISMA infant forms

*Value:* Among liveborn infants with PNC visits marked as complete, percent that have all expected visit forms completed. A visit is considered `complete` if the data reports the visit was conducted in person or by phone (variable name: `MAT_VISIT_MNHXX`).

*Goal:* 100% completion for all forms.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (variable name: ``MAT_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i

*Denominator:* Any form with visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (variable name: `MAT_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i. *For PNC52 visits denominator is passed late window for visit type = i AND has not closed out OR closed out with reason for closeout=1-year followup completed).*
...
table12_1 <- inf_prot_compliance_pnc[c(1:9),]
table12_2 <- inf_prot_compliance_pnc[c(1,10:25),]


## PART 1
prisma_tab12_1 <- tb_theme1(table12_1) %>% 
  tab_header(
...
## PART 2
prisma_tab12_2 <- tb_theme1(table12_2) %>% 
  tab_header(
...
```{r}
prisma_tab12_1 <- prisma_tab12_1 %>% gtsave("prisma_tab12_1.png")
knitr::include_graphics("prisma_tab12_1.png")

```

```{r}
prisma_tab12_2 <- prisma_tab12_2 %>% gtsave("prisma_tab12_2.png")
knitr::include_graphics("prisma_tab12_2.png")

```

\newpage

### Table 13: PNC visit infant form completion for PRISMA

*Value:* Among liveborn infants who have passed the late PNC window period, percent with forms completed regardless of visit status. A form is considered complete when a form has been submitted for a participant.
...
table13_1 <- inf_form_comp_pnc[c(1:9),]
table13_2 <- inf_form_comp_pnc[c(1,10:25),]
...
### Table 14. Summary of PRISMA infant outcomes.
...
### Table 15. GSED and FCI in PRISMA/ReMAPP Cohort
...
gsed_sf_tab <- tb_theme1(gsed_sf_tab_data) %>%
  tab_header(
    title = md("Table 15. GSED and FCI in PRISMA/ReMAPP Cohort")
  ) %>%
  tab_row_group(
...
<!-- ### NOT YET INCLUDING: Table 17. ReMIND Assessment Completion -->

<!-- *Value:* Are forms completed for each visit? A visit is considered complete if the data reports the visit was conducted in person or by phone (variable name: MAT_VISIT_MNHXX ). If there is no late window for a visit, the on-time window is used. -->

<!-- *Numerator:* Aim 3 enrolled livebirth with visit date within visit window AND have complete visit status (where variable name: `MAT_VISIT_MNHXX`=1 or 2) AND passed late window  -->

<!-- *Denominator:* Aim 3 enrolled livebirth passed late window AND (has not closed out OR closed out with closeout date \> visit window). *For PNC52 visits denominator is passed late window for visit type = i AND has not closed out OR closed out with reason for closeout=1-year followup completed).* -->
