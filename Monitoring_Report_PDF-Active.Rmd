---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2026-03-24)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
mat_enroll <- read.csv(paste0("~/import/MAT_ENROLL.csv"))
remapp_ids <- mat_enroll %>% filter(REMAPP_ENROLL == 1)

setwd(paste0("D:/Users/stacie.loisate/Box/Monitoring-Report-Active/data/"))
# load(paste0("D:/Users/stacie.loisate/Box/Monitoring-Report-Active/data/MatData_Screen_Enroll.RData"))
...
**Included data last updated in Synapse:** 2026-03-20
...
## input date of last enrollment above under "Last enrollment date:"
...
### Table 1: Cumulative pre-screened for PRISMA
...
### Table 2: Cumulative enrolled in PRISMA
...
### Table 3: Overview of PRISMA participant study status
...
### Table 4: ANC and PNC visit completion for PRISMA
...
### Table 5: ANC protocol compliance for PRISMA
...
prisma_tab6_1 <- tb_theme1(table6_1) %>% 
  tab_header(
    title = md("**PRISMA Table 5** *(continued on next page)*")
  ) %>%
  tab_row_group(
...
prisma_tab6_2 <- tb_theme1(table6_2) %>% 
  tab_header(
    title = md("**PRISMA Table 5** *(continued)*")
  ) %>%
  tab_row_group(
...
### Table 6: ANC form completion for PRISMA
...
### Table 7: IPC visit completion and PRISMA protocol compliance for IPC
...
```{r, echo=FALSE, out.width="60%", out.height="30%", fig.show='hold', fig.align='left'}
...
### Table 8: PNC protocol compliance for PRISMA maternal forms
...
prisma_tab9_1 <- tb_theme1(table9_1) %>% 
  tab_header(
    title = md("**PRISMA Table 8** *(continued on next page)*")
  ) %>%
  tab_row_group(
...
prisma_tab9_2 <- tb_theme1(table9_2) %>% 
  tab_header(
    title = md("**PRISMA Table 8** *(continued)*")
  ) %>%
  tab_row_group(
...
### Table 9: PNC visit maternal form completion for PRISMA
...
### Table 10: Protocol compliance for PRISMA depression assessment
...
out <- heat_map_data_anc %>%
  ...
  geom_tile(color = "grey40", show.legend = TRUE)  +
...
### Table 11: Infant PNC visit completion for PRISMA
...
### Table 12: Infant PNC protocol compliance for PRISMA
...
prisma_tab13_1 <- tb_theme1(table13_1) %>% 
  tab_header(
    title = md("**PRISMA Table 12** *(continued on next page)*")
  ) %>%
  tab_row_group(
...
prisma_tab13_2 <- tb_theme1(table13_2) %>% 
  tab_header(
    title = md("**PRISMA Table 12** *(continued)*")
  ) %>%
  tab_row_group(
...
### Table 13: Infant PNC form completion for PRISMA
...
<!-- REMOVE FOR NOW, BUT CAN ADD BACK IN AS NEEDED ### Table 14 Summary of PRISMA infant outcomes. -->
...
### Table 14 GSED and FCI in PRISMA/ReMAPP Cohort
...
gsed_sf_tab <- tb_theme1(gsed_sf_tab_data) %>%
  tab_header(
    title = md("Table 14 GSED and FCI in PRISMA/ReMAPP Cohort")
  ) %>%
  tab_row_group(
...
### Table 15: Bili-ruler (MNH36) Visit and Form Completion
...
table17_tab <- tb_theme1(table17) %>% 
  tab_header(
    title = md("**Table 15: Bili-ruler Form and Visit Completion**")
  ) %>%
  tab_row_group(
...
### Table 16: Protocol compliance for ReMAPP fatigue assessment
...
### Table 17: Participant eligibility for the ReMAPP healthy cohort
...
### Table 18: Protocol compliance for ReMAPP hemoglobin measurements by CBC
...
gsed_aim3_summary_tab <- tb_theme1(gsed_aim3_tab_summary_labels) %>%
  tab_header(
    title = md("Table 19 ReMIND Study Status")
  ) %>%
...
### Table 20 ReMIND Assessment Completion
...
gsed_aim3_tab <- tb_theme1(gsed_aim3_tab_data) %>%
  tab_header(
    title = md("Table 20. ReMIND Assessment Completion")
  ) %>%
  tab_row_group(
