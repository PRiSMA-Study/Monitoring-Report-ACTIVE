---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2025-02-25)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
**Included data last updated in Synapse:** 2025-02-21
...
-   Ghana: 2025-02-19
-   Zambia: 2025-02-12
-   India CMC: 2025-02-07
-   India SAS: 2025-02-18
...
out <- heat_map_data_anc %>% 
  ...
  geom_tile(color = "grey40")  + # ,size = 0.2, height = 2, width = 0.9) +  # width = 0.9, 
...
*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have a complete visit status (variable name: \``MAT_VISIT_MNHXX`=1, 2,
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> Denominator is all live births AND have passed the risk period with a visit OR died within the risk period (risk period: age = 28days).</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> Denominator is all live births AND have passed the risk period (risk period: 365days past birth).</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> Denominator is all livebirths and stillbirths.</span>")
 ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> Denominator is all infants with a fetal loss reported in MNH04 `(PRG_DSDECOD)` or birth outcome in MNH09 `(BIRTH_DSTERM_INF1)`.</span>")
 ) %>% 
  tab_footnote(
...
All infants with a jaundice assessment at <14 weeks of life.</span>")
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>g</sup> Denominator is all livebirths aged 0-59 days.</span>")
 ) 
...
gsed_sf_tab <- tb_theme1(gsed_sf_tab_data) %>%
  ...
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>a</sup> denominator is n passed late window with >=17 weeks gestation AND (has not closed out OR closed out with closeout date >=26wks).</span>")
  ) %>% 
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>b</sup> denominator is n passed late window with >=30 weeks gestation AND (has not closed out OR closed out with closeout date >=52wks).</span>")
  ) %>% 
tab_footnote(
  footnote = md("<span style='font-size: 18px'><sup>c</sup> denominator is n passed late window with >=56 weeks gestation OR closed out with closeout reason reported as: *1-year postpartum follow-up period has ended*.</span>")
  ) 
