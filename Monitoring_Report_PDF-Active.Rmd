---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2025-12-16)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
tb_theme1 <- function(matrix){
  tb <- matrix %>% 
    ...
    cols_width(1 ~ px(225)) ## to make rows more spread out, reduce this number 
} 
...
replace_all_sas <- function(df) {
  df <- df 
  return(df)
}

MatData_Screen_Enroll <- MatData_Screen_Enroll %>% 
  left_join(mat_enroll %>%  select(SITE, SCRNID, MOMID, PREGID, US_GA_WKS_ENROLL, US_GA_DAYS_ENROLL), by = c("SITE", "SCRNID", "MOMID", "PREGID"))
...
**Included data last updated in Synapse:** 2025-12-12
...
-   Ghana: 2025-04-22
-   Zambia: 2025-03-20
-   India CMC: 2025-11-28
-   India SAS: 2025-03-04

```{r, results='hide' }

#check last day of screening and enrollment
lastenroll <- MatData_Screen_Enroll %>%
  mutate(M02_SCRN_OBSSTDAT = ymd(M02_SCRN_OBSSTDAT)) %>%
  filter(M02_SCRN_OBSSTDAT  <= UploadDate) %>%
  filter(ENROLL == 1) %>%
  select(M02_SCRN_OBSSTDAT , SITE) %>%
  group_by(SITE) %>%
...
tb_prescreen_enroll <- tb_prescreen_enroll %>% 
...
tb_prescreen_enroll <- replace_all_sas(tb_prescreen_enroll)
...
# webshot2::webshot("prisma_tab1.html", "prisma_tab1.png")
...
*Value:* Distribution of total pre-screened since the launch of the
harmonized PRISMA protocol. If ineligible based on pre-screening,
distributions by exclusion reason are listed.
...
tb_prescreen_all <- replace_all_sas(tb_prescreen_all)
...
*Value:* Distribution by exclusion criteria among those who are
ineligible in pre-screening are listed below. Each row should sum to
100%.

```{r, echo=FALSE}
### STACKED BAR CHART
total_bysite_pre <- MatData_Screen_Enroll %>%
     # filter(SITE != "India-SAS") %>%
  filter(PRESCREEN == 1 & PRESCREEN_DENOM == 1 & PRESCR_ELIGIBLE==0) %>% #  & SITE != "India-SAS"
...
total_allsites_pre <- MatData_Screen_Enroll %>%
  # filter(SITE != "India-SAS") %>%
  filter(PRESCREEN == 1 & PRESCREEN_DENOM == 1 & PRESCR_ELIGIBLE==0) %>% #  & SITE != "India-SAS")
  select(SITE, PRESCR_PREGSIGN, PRESCR_GA25, PRESCR_AGE, PRESCR_CATCHAREA, PRESCR_OTHER,CONSENT_PRESCREEN) %>%
  rename( "No clinical signs of pregnancy" = PRESCR_PREGSIGN,
...
total_inelg = MatData_Screen_Enroll %>%
  filter(PRESCR_ELIGIBLE == 0) %>% #  & SITE != "India-SAS"
  select(SITE, PRESCR_ELIGIBLE) %>%
  group_by(SITE) %>%
  summarise(n_ineligible = sum(PRESCR_ELIGIBLE==0, na.rm = TRUE))

barchart_data_out <- bind_rows(total_bysite_pre, total_allsites_pre) %>%
  # filter(SITE != "India-SAS") %>%
...
geom_text(aes(label = n_ineligible, x = SITE, y = n_pct+0.25),
  ...
  coord_flip() +
  scale_fill_manual(values = c("#b40e20","#0c775e", "#e2d300", "#47acc8", "#915F6D","#e58600"), ##"pink",
...
*Value:* Distribution of cumulative enrollment and study progress
distributions since the start of relaunch. If ineligible, distributions
by exclusion criteria are listed below. Each column should sum to 100%.
...
tb_enroll <- MatData_Screen_Enroll %>%
  ...
  summarise("Screened, n" = sum(SCREEN == 1, na.rm=TRUE),
            ...
            "Enrolled, n(%)^b^" = paste0(sum(ENROLL == 1, na.rm=TRUE),
                                        " (", format(sum(ENROLL == 1, na.rm=TRUE)/sum(SCREEN == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
    ...
    ")"),
          "Did not return for screening^d^" = paste0(sum(SCRN_RETURN == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                             " (", format(sum(SCRN_RETURN == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
            
          "Did not meet age requirement" = paste0(sum(AGE == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                             " (", format(sum(AGE == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
          "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)" = paste0(sum(GA20 == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                                " (", format(sum(GA20 == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
          "Outside catchment area" = paste0(sum(CATCHAREA == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                                   " (", format(sum(CATCHAREA == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
          "Doesn't plan to remain in catchment area" = paste0(sum(CATCHREMAIN == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                                   " (", format(sum(CATCHREMAIN == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")"),
          "Did not consent" = paste0(sum(SCRN_CONSENT == 0 & SCREEN_DENOM == 1, na.rm=TRUE),
                                  " (", format(sum(SCRN_CONSENT == 0 & SCREEN_DENOM == 1, na.rm=TRUE)/sum(SCREEN_DENOM == 1, na.rm=TRUE)*100, digits = 2, nsmall=0), ")")
  ) %>%
    t() %>% as.data.frame() %>%
  `colnames<-`(c(.[1,])) %>%
  slice(-1) %>%
...
tb_enroll <- replace_all_sas(tb_enroll)
...
*Value:* Distribution by exclusion criteria among those who are
ineligible in screening are listed below. Each row should sum to 100%.
...
### STACKED BAR CHART
  filter(SCREEN == 1 & SCREEN_DENOM == 1 & is.na(ENROLL)) %>%
    filter(SCRNID != "") %>%
  select(SITE,SCRN_RETURN, AGE, GA20, CATCHAREA, CATCHREMAIN, SCRN_CONSENT) %>%
  #   left_join(mnh02_ke %>% mutate(SCRNID = as.character(SCRNID)) %>%
  #             select(SCRNID, SCRN_RETURN) %>%
  #             rename(NEW_SCRN_RETURN = SCRN_RETURN),
  #           by = c("SCRNID")) %>%
  # mutate(SCRN_RETURN = case_when(SITE == "Kenya" ~ NEW_SCRN_RETURN, TRUE ~ SCRN_RETURN)) %>%
  # select(-SCRNID, -NEW_SCRN_RETURN) %>%
  # mutate(CONSENT = case_when(SCRN_RETURN !=0 & AGE != 0 & GA20!=0 &
  #                             CATCHAREA !=0  & CATCHREMAIN!=0 & CONSENT==0 ~ 0, TRUE ~ 99)) %>%
...
total_inelg = MatData_Screen_Enroll %>% 
  select(SITE, ENROLL, SCREEN_DENOM) %>% 
...
ggplot(barchart_data_out %>%  mutate(SITE = factor(SITE, levels = desired_order)), 
  ...
  geom_text(aes(label = n_ineligible, x = SITE, y = n_pct+0.25), 
    ...
    coord_flip() +  
    scale_fill_manual(values = c("#b40e20", "#0c775e","#e2d300", "#47acc8","#915F6D", "#e58600"), #, "#e58600"
...
<!--   scale_color_manual(values = c("#F8766D", "#A3A500","orange", "#00BF7D", "#00B0F6", "#E76BF3"), # -->

<!--                      labels = c("Ghana", "India-CMC", "India-SAS", "Kenya", "Pakistan", "Zambia")) + # -->
...
### Figure 3: Cumulative actual versus expected enrollment in PRISMA by site

*Value:* Compare expected and actual enrollment for each site over time.
Calculated by dividing the total target enrollment by 36 months to get
the average enrollment per month over the course of three years (one
year for India-SAS). We can further divide to get average enrollment per
week.

```{r, echo=FALSE, out.width="49%", out.height="25%", fig.show='hold', fig.align='left'}
...
enroll_cum_PA <- MatData_Screen_Enroll %>% 
  ...
  summarise(enroll_n=sum(ENROLL==1, na.rm = TRUE),
...
enroll_cum_PA %>% 
  ...
  facet_grid(~year(START_DATE_MOM_WK), space = "free_x", scales = "free_x", switch= "x") + 
  # facet_wrap(~ START_DATE_MOM_WK, scales = "free_y")
  theme_bw() +
...
enroll_cum_KE <- MatData_Screen_Enroll %>% 
  ...
  mutate(enroll_n = case_when(is.na(enroll_n) ~ 0, TRUE ~ enroll_n)) %>% 
  mutate(expect_cum=cumsum(expect_n),
         enroll_cum=cumsum(enroll_n))%>% 
  ungroup() 
...
enroll_cum_SAS <- MatData_Screen_Enroll %>%
  ...
  ## calculate number of weeks between start and stop dates since SAS is only until 2000 participants
  mutate(WEEKS_SAS_ENROLL = as.numeric(difftime(ymd("2025-03-06"), ymd("2023-12-12"), units = "weeks")),
         DAYS_SAS_ENROLL = WEEKS_SAS_ENROLL*7
         ) %>%

  group_by(START_DATE_MOM_WK) %>%
  mutate(EXPECT = 2000/DAYS_SAS_ENROLL) %>%
  summarise(enroll_n=sum(ENROLL==1, na.rm = TRUE),
            expect_n=mean(EXPECT)) %>%
  mutate(
  gap = as.numeric(ymd(START_DATE_MOM_WK)-lag(ymd(START_DATE_MOM_WK))),
...
*Value:* Distribution of current status among enrolled women. Each
...
rm(MatData_Screen_Enroll)
```

```{r}
tb_status <- replace_all_sas(tb_status)
...
*Value:* Are forms completed for each visit? A visit is considered
complete if the data reports the visit was conducted in person or by
phone (variable name: MAT_VISIT_MNHXX ). If there is no late window for
a visit, the on-time window is used.

*Goal:* 100% completion for enrollment; 90% completion for all other
visits

*Numerator:* Any form with visit type = i (variable name: `TYPE_VISIT`)
AND have a complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i
...
*Value:* Among women successfully contacted during the ANC period,
percent who had all expected visit forms completed. A visit is
considered `complete` if the data reports the visit was conducted in
person or by phone (variable name: `MAT_VISIT_MNHXX`).

*Goal:* 100% completion for all forms, except MNH08 where lab results
are expected to lag.
...
*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have a complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i
...
ANC32 <- Prot_Compliance_Anc %>% 
  ...
  "Denominator" = paste0(
   ...
   "MNH01 Ultrasound" = paste0(
        format(sum(((M01_VISIT_COMPLETE_4 == 1 & ANC32_PASS_LATE == 1) |
                        (M01_VISIT_COMPLETE_5 == 1 & ANC32_PASS_LATE == 1) & PC_ANC32_DENOM==1), na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
        format(round(sum(((M01_VISIT_COMPLETE_4 == 1 & ANC32_PASS_LATE == 1) |
                        (M01_VISIT_COMPLETE_5 == 1 & ANC32_PASS_LATE == 1) & PC_ANC32_DENOM==1), na.rm = TRUE)/sum(PC_ANC32_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
        ")"),
...
prot_compliance_anc <- bind_rows(ga_tb, 
 ...
 prot_compliance_anc <- replace_all_sas(prot_compliance_anc)
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> denominator is n completed any form with visit status=complete AND visit type=ANC-32  AND passed late window with>33 weeks gestation. Ultrasounds completed during the ANC36 on time window are included.</span>")
) %>% 
tab_footnote(
...
*Value:* Among women who have passed the late ANC window period, percent
with forms completed regardless of visit status. A form is considered
complete when a form has been submitted for a participant.

*Goal:* 100% completion for all forms, except MNH08 where lab results
are expected to lag.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have any visit status (variable name: `MAT_VISIT_MNHXX`) AND passed late
window for visit type = i

*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have any visit status (variable name:
`MAT_VISIT_MNHXX`) AND passed late window for visit type = i
...
form_completion_anc <- replace_all_sas(form_completion_anc)
...
*Value:* Among women successfully contacted during the IPC period,
percent who have expected visit forms completed. A visit is considered
`complete` if the data reports the visit was conducted in person or by
phone (variable name: `MAT_VISIT_MNHXX`).
...
*Value:* Gestational age of participants who have passed the target IPC
window and do not have any IPC forms.
...
figure2 <- MatData_Ipc_Visits_Mat %>%
  ...
  select(SITE, MOMID, PREGID,DOB, PREG_START_DATE, CLOSED_OUT, M23_CLOSE_DSSTDAT) %>%
...
#                                                "estimated date of conception. calculated using teh date of enrollment ultrasound and gestational age at enrollment (PREG_START_DATE = ultrasound date - GA at ultrasound).",
...
*Value:* Among women with PNC visits marked as complete, percent that
have all expected visit forms completed. A visit is considered
`complete` if the data reports the visit was conducted in person or by
phone (variable name: `MAT_VISIT_MNHXX`).
...
*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have a complete visit status (variable name: \``MAT_VISIT_MNHXX`=1 or 2)
AND passed late window for visit type = i

*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have a complete visit status (variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i.
*For PNC52 visits denominator is passed late window for visit type = i
AND has not closed out OR closed out with reason for closeout=1-year
followup completed).*
...
prot_compliance_pnc <- replace_all_sas(prot_compliance_pnc)
...
*Value:* Among women who have passed the late PNC window period, percent
with forms completed regardless of visit status. A form is considered
complete when a form has been submitted for a participant.
...
*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have any visit status (variable name:
`MAT_VISIT_MNHXX`) AND passed late window for visit type = i. *For PNC52
visits denominator is passed late window for visit type = i AND has not
closed out OR closed out with reason for closeout=1-year followup
completed).*
...
form_completion_pnc <- replace_all_sas(form_completion_pnc)
...
*Value:* Among women successfully contacted during the ANC or PNC period
who have passed the expected visit window, percent that have MNH25
complete. A visit is considered `complete` if the data reports the visit
was conducted in person or by phone (variable name: `MAT_VISIT_MNHXX`).
...
prisma_table11 <- replace_all_sas(prisma_table11)
## table 8
...
### Figure 5. Heat map of form completion for maternal ANC visits in PRISMA
...
ANC20_PCT <- Heat_Maps_Anc %>% 
  ...
  mutate(PCT_BIN = cut(COMPLETE_PCT, 
                       ...
                       include.lowest = TRUE,
                       labels = c(paste(seq(0, 80, by = 10), seq(9, 90, by = 10), sep = "-"), "90-100"))) #%>% 
  # mutate(PCT_BIN = case_when(FORM %in% c("MNH01", "MNH02", "MNH03") ~ "Form not required", TRUE ~ PCT_BIN))
...
ANC32_PCT <- Heat_Maps_Anc %>% 
  ...
  mutate(PCT_BIN = cut(COMPLETE_PCT, 
                       ...
                       include.lowest = TRUE,
                       labels = c(paste(seq(0, 80, by = 10), seq(9, 90, by = 10), sep = "-"), "90-100")))# %>% 
  # mutate(PCT_BIN = case_when(FORM %in% c("MNH02", "MNH03") ~ "Form not required", TRUE ~ PCT_BIN))
...
heat_map_data_anc <- heat_map_data_anc %>% 
  mutate(PCT_BIN = case_when(FORM %in% c("MNH01", "MNH02", "MNH03") & VISIT %in% c("ANC20", "ANC28", "ANC36")~ "Form not required", TRUE ~ PCT_BIN),
         PCT_BIN = case_when(FORM %in% c("MNH02", "MNH03") & VISIT %in% c("ANC32")~ "Form not required", TRUE ~ PCT_BIN)
         )
...
custom_palette <- colorRampPalette(c("grey50", "darkred", "#B22222", "#FF0000", "#FF6347", "#F9A603",
                                     "#FFA500", "#FFD700", "#FFFF00", "#7bd386", "darkgreen"))(11)

UNIQUE_VISITS = as.vector(unique(heat_map_data_anc$VISIT))

# set levels for percent bins
heat_map_data_anc$PCT_BIN <- factor(heat_map_data_anc$PCT_BIN, levels = c("Form not required", "0-9", "10-19", "20-29", "30-39", "40-49",
                               "50-59", "60-69", "70-79", "80-89", "90-100"))
# set levels for visits
heat_map_data_anc$VISIT <- factor(heat_map_data_anc$VISIT, levels = c("Enrollment", "ANC20", "ANC28", "ANC32", "ANC36"))

# heat_map_data_anc <- heat_map_data_anc %>% filter(SITE != "India-SAS")

out <- heat_map_data_anc %>%
  mutate(SITE = factor(SITE, levels = rev(levels(factor(SITE))))) %>%  # Reverse the order of levels
  ggplot(aes(x = FORM, y = SITE, fill = PCT_BIN)) +
  geom_tile(color = "grey40", show.legend = TRUE)  + # ,size = 0.2, height = 2, width = 0.9) +  # width = 0.9, 
  facet_grid(rows = vars(VISIT)) +
  scale_fill_manual(values = custom_palette,
                    labels = str_wrap(c("Form not required", "0-9%", "10-19%", "20-29%", "30-39%", "40-49%",
                                        "50-59%", "60-69%", "70-79%", "80-89%", "90-100%"), width = 10), 
                    drop = FALSE) +
  ...
  theme(legend.position = "bottom",
        ...
        # strip.text.y = element_text(size = 8, margin = margin(t = 11, b = 10)),  # Space for facet labels
  ...
  coord_fixed(ratio = 0.175, expand = TRUE)  # Adjust the ratio to make the tiles shorter 0.35


  # scale_fill_manual(values = custom_palette,
  #                   labels = c("Form not required", "0-9%", "10-19%", "20-29%", "30-39%", "40-49%",
  #                              "50-59%", "60-69%", "70-79%", "80-89%", "90-100%"),
  #                   drop = FALSE) +
  # labs(fill = str_wrap("Percent Complete", width = 8)) +
  # xlab("Expected forms") +
...
```{r,fig.height=7, fig.width=6, fig.pos="H"}
out
```

\newpage

### Figure 6. Heat maps of form completion for maternal PNC visits in PRISMA
...
out OR closed out with closeout date \> visit window). *For PNC52 visits
denominator is passed late window for visit type = i AND has not closed
out OR closed out with reason for closeout=1-year followup completed).*
...
heat_map_data_pnc <- heat_map_data_pnc %>% 
  mutate(PCT_BIN = case_when(FORM %in% c("MNH05", "MNH07", "MNH08") & VISIT %in% c("PNC0", "PNC1", "PNC4")~ "Form not required", TRUE ~ PCT_BIN),
         PCT_BIN = case_when(FORM %in% c("MNH07", "MNH08") & VISIT %in% c("PNC26", "PNC52")~ "Form not required", TRUE ~ PCT_BIN)
         )
...
heat_map_data_pnc <- heat_map_data_pnc %>%
  ...
  mutate(
    PCT_BIN = factor(PCT_BIN, levels = c("Form not required", "0-9", "10-19", "20-29", "30-39", "40-49",
                                         "50-59", "60-69", "70-79", "80-89", "90-100")),
    VISIT = factor(VISIT, levels = c("PNC0", "PNC1", "PNC4", "PNC6", "PNC26", "PNC52"))
  )

# heat_map_data_pnc <- heat_map_data_pnc %>% filter(!is.na(PCT_BIN))
# # set levels for percent bins
# heat_map_data_pnc$PCT_BIN <- factor(heat_map_data_pnc$PCT_BIN, levels = c("Form not required", "0-9", "10-19", "20-29", "30-39", "40-49",
#                                "50-59", "60-69", "70-79", "80-89", "90-100"))
# # set levels for visits
# heat_map_data_pnc$VISIT <- factor(heat_map_data_pnc$VISIT, levels = c("PNC0", "PNC1", "PNC4", "PNC6", "PNC26", "PNC52"))

# heat_map_data_pnc <- heat_map_data_pnc %>%   filter(SITE != "India-SAS")
out <- heat_map_data_pnc %>% 
  mutate(SITE = factor(SITE, levels = rev(levels(factor(SITE))))) %>%  # Reverse the order of levels
  ggplot(aes(x = FORM, y = SITE, fill = PCT_BIN)) +
  geom_tile(color = "grey40", show.legend = TRUE)  + # ,size = 0.2, height = 2, width = 0.9) +  # width = 0.9, 
  facet_grid(rows = vars(VISIT)) +
...
### Table 12: Infant PNC visit completion for PRISMA

*Value:* Among liveborn infants with PNC visits, are forms completed for
each visit? A visit is considered complete if the data reports the visit
was conducted in person or by phone (variable name: INF_VISIT_MNHXX ).
If there is no late window for a visit, the on-time window is used.

*Numerator:* Any form with visit type = i (variable name: `TYPE_VISIT`)
AND have a complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i
...
## PNC
INF_VISIT_COMPLETE_PNC <- Inf_Visit_Complete_Pnc %>%
  rowwise() %>%
  group_by(SITE) %>%
  summarise(
...
inf_visit_complete <- INF_VISIT_COMPLETE_PNC %>%
    `rownames<-` (c("PNC-0 (3 to 5 days)", ## NONE - SKIP WINDOW
                 "Denominator,n",
                 "PNC-1 (7 to 14 days)", ## NONE - SKIP WINDOW
                 "Denominator,n ",
                 "PNC-4 (28 to 35 days)", ## NONE - SKIP WINDOW
...
inf_visit_complete <- replace_all_sas(inf_visit_complete)

inf_visit_complete_tab <- tb_theme1(inf_visit_complete) %>% 
  tab_header(
...
```{r}
inf_visit_complete_tab <- inf_visit_complete_tab %>% gtsave("inf_visit_complete_tab.png")
knitr::include_graphics("inf_visit_complete_tab.png")

```

\newpage

### Table 13: Infant PNC protocol compliance for PRISMA

*Value:* Among liveborn infants with PNC visits marked as complete,
percent that have all expected visit forms completed. A visit is
considered `complete` if the data reports the visit was conducted in
person or by phone (variable name: `INF_VISIT_MNHXX`).

*Goal:* 100% completion for all forms.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have a complete visit status (variable name: \``INF_VISIT_MNHXX`=1, 2,
or 3) AND passed late window for visit type = i

*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have a complete visit status (variable name:
`INF_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i.
*For PNC52 visits denominator is passed late window for visit type = i
AND has not closed out OR closed out with reason for closeout=1-year
followup completed).*
...
birthoutcome <- InfData_Pnc_Visits %>%
  group_by(SITE) %>%
  summarise(
    "Total livebirths" = paste0(
      "(n=",sum(LIVEBIRTH==1, na.rm = TRUE), ")"
    )
  ) %>% ungroup() %>%
  t() %>%
  as.data.frame() %>%
...
PNC0 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_7 == 1 & PC_PNC0_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_7 == 1 & PC_PNC0_DENOM == 1, na.rm = TRUE)/sum(PC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1, na.rm = TRUE)/sum(PC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1, na.rm = TRUE)/sum(PC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
    ) %>%
  mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  ...
  add_column(
    .before = 1,
    "Total" = Inf_Prot_Compliance_Pnc %>%
    
    mutate(M15_VISIT_COMPLETE_7 = case_when(SITE == "Pakistan" ~ NA, TRUE ~ M15_VISIT_COMPLETE_7)) %>% 
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_7==1 & PC_PNC0_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC0_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
      t() %>%
      as.data.frame() %>%
...
PNC1 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_8 == 1 & PC_PNC1_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_8 == 1 & PC_PNC1_DENOM == 1, na.rm = TRUE)/sum(PC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1, na.rm = TRUE)/sum(PC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1, na.rm = TRUE)/sum(PC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ) %>%
    mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  t() %>% as.data.frame() %>%
  ...
  add_column(
    ...
    "Total" = Inf_Prot_Compliance_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_8==1 & PC_PNC1_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC1_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
...
PNC4 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_9 == 1 & PC_PNC4_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_9 == 1 & PC_PNC4_DENOM == 1, na.rm = TRUE)/sum(PC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1, na.rm = TRUE)/sum(PC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1, na.rm = TRUE)/sum(PC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ) %>%
    mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  ...
  add_column(
    ...
    "Total" = Inf_Prot_Compliance_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_9==1 & PC_PNC4_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC4_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
...
PNC6 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_10 == 1 & PC_PNC6_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_10 == 1 & PC_PNC6_DENOM == 1, na.rm = TRUE)/sum(PC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1, na.rm = TRUE)/sum(PC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1, na.rm = TRUE)/sum(PC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ) %>%
    mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  ...
  add_column(
    ...
    "Total" = Inf_Prot_Compliance_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_10==1 & PC_PNC6_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC6_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
...
PNC26 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_11 == 1 & PC_PNC26_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_11 == 1 & PC_PNC26_DENOM == 1, na.rm = TRUE)/sum(PC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1, na.rm = TRUE)/sum(PC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1, na.rm = TRUE)/sum(PC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ) %>%
    mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  ...
  add_column(
    ...
    "Total" = Inf_Prot_Compliance_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_11==1 & PC_PNC26_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC26_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
...
PNC52 <- Inf_Prot_Compliance_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      format(sum(M13_VISIT_COMPLETE_12 == 1 & PC_PNC52_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M13_VISIT_COMPLETE_12 == 1 & PC_PNC52_DENOM == 1, na.rm = TRUE)/sum(PC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(M14_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M14_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1, na.rm = TRUE)/sum(PC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(M15_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(M15_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1, na.rm = TRUE)/sum(PC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ) %>%
    mutate(`MNH15 Infant Vaccination` = case_when(SITE == "Pakistan" ~ "-- (-)", TRUE ~ `MNH15 Infant Vaccination` )) %>% 
  ...
  add_column(
    ...
    "Total" = Inf_Prot_Compliance_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(M15_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(M15_VISIT_COMPLETE_12==1 & PC_PNC52_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(PC_PNC52_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
      ) %>% 
...
inf_prot_compliance_pnc <- bind_rows(birthoutcome,
                               PNC0,
                               PNC1, 
                               PNC4, 
                               PNC6, 
  ...
  `rownames<-`(c("Livebirths, n",
                 ## PNC0 (2-5)
                 "Denominator, n",
                 "MNH13 Infant Clinical Status",
                 "MNH14 Infant POC",
                 "MNH15 Infant Vaccination**",
                 ## PNC1 (6-9)
                 "Denominator, n ",
                 "MNH13 Infant Clinical Status ",
                 "MNH14 Infant POC ",
...
inf_prot_compliance_pnc <- replace_all_sas(inf_prot_compliance_pnc)

table13_1 <- inf_prot_compliance_pnc[c(1:9),]
table13_2 <- inf_prot_compliance_pnc[c(1,10:25),]


## PART 1
prisma_tab13_1 <- tb_theme1(table13_1) %>% 
  tab_header(
    title = md("**PRISMA Table 13** *(continued on next page)*")
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-0 visit, n(%)<sup>a</sup> [Target window: 3 to 5 days]</span>"),
    rows = 2:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-1 visit, n(%)<sup>b</sup> [Target window: 7 to 14 days]</span>"),
    rows = 6:9
  ) %>%
  
  row_group_order(groups = c(NA,
                             "<span style='font-size: 18px'>PNC-0 visit, n(%)<sup>a</sup> [Target window: 3 to 5 days]</span>",
                             "<span style='font-size: 18px'>PNC-1 visit, n(%)<sup>b</sup> [Target window: 7 to 14 days]</span>")) %>%
  tab_style(
    style = list(
      cell_text(weight = "bold")
    ),
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> denominator is n completed any form with visit status=complete AND visit type=PNC-0 AND passed late window with age >5 days.</span>")
  ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n completed any form with visit status=complete AND visit type=PNC-1 AND passed late window with age >14 days.</span>")  
  ) %>% 
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>**</sup> Total denominator does not include Pakistan.</span>")  
  )   
 

## PART 2
prisma_tab13_2 <- tb_theme1(table13_2) %>% 
  tab_header(
    title = md("**PRISMA Table 13** *(continued)*")
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-4 visit, n(%)<sup>c</sup> [Target window: 28 to 35 days]</span>"),
    rows = 2:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-6 visit, n(%)<sup>d</sup> [Target window: 6 to 12 wks]</span>"),
    rows = 6:9
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-26 visit, n(%)<sup>e</sup> [Target window: 26 to 39 wks]</span>"),
    rows = 10:13
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-52 visit, n(%)<sup>f</sup> [Target window: 52 to 64 wks]</span>"),
    rows = 14:17
  ) %>%
  
  row_group_order(groups = c(NA,
                             "<span style='font-size: 18px'>PNC-4 visit, n(%)<sup>c</sup> [Target window: 28 to 35 days]</span>",
                             "<span style='font-size: 18px'>PNC-6 visit, n(%)<sup>d</sup> [Target window: 6 to 12 wks]</span>",
                             "<span style='font-size: 18px'>PNC-26 visit, n(%)<sup>e</sup> [Target window: 26 to 39 wks]</span>",
                             "<span style='font-size: 18px'>PNC-52 visit, n(%)<sup>f</sup> [Target window: 52 to 64 wks]</span>")) %>%
  tab_style(
    style = list(
      cell_text(weight = "bold")
    ),
  ...
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> denominator is n completed any form with visit status=complete AND visit type=PNC-4 AND passed late window with age >35 days.</span>")
  ) %>%
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> denominator is n completed any form with visit status=complete AND visit type=PNC-6 AND passed late window with age >12 wks.</span>")
  ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> denominator is n completed any form with visit status=complete AND visit type=PNC-26 AND passed late window with age >39 wks.</span>")  
  ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>f</sup> denominator is n completed any form wit visit status=complete AND visit type=PNC-52 AND passed late window with age >64 wks.</span>")  
  ) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>Note</sup> passed window denominators for all visits are calculated by date and time of birth reported in MNH09. If a date of birth is missing, the particpant is removed from this table.</span>")  
  )  %>% 
...
```{r}
prisma_tab13_1 <- prisma_tab13_1 %>% gtsave("prisma_tab13_1.png")
knitr::include_graphics("prisma_tab13_1.png")

```

```{r}
prisma_tab13_2 <- prisma_tab13_2 %>% gtsave("prisma_tab13_2.png")
knitr::include_graphics("prisma_tab13_2.png")

```

\newpage

### Table 14: Infant PNC form completion for PRISMA

*Value:* Among liveborn infants who have passed the late PNC window
period, percent with forms completed regardless of visit status. A form
is considered complete when a form has been submitted for a participant.

*Goal:* 100% completion for all forms.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have any visit status (variable name: `INF_VISIT_MNHXX`) AND passed late
window for visit type = i

*Denominator:* Any form with visit type = i (variable name:
`TYPE_VISIT`) AND have any visit status (variable name:
`INF_VISIT_MNHXX`) AND passed late window for visit type = i. *For PNC52
visits denominator is passed late window for visit type = i AND has not
closed out OR closed out with reason for closeout=1-year followup
completed).*
...
PNC0 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1, na.rm = TRUE)/sum(FC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1, na.rm = TRUE)/sum(FC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1, na.rm = TRUE)/sum(FC_PNC0_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_7) & FC_PNC0_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC0_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
PNC1 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1, na.rm = TRUE)/sum(FC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1, na.rm = TRUE)/sum(FC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1, na.rm = TRUE)/sum(FC_PNC1_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_8) & FC_PNC1_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC1_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
PNC4 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1, na.rm = TRUE)/sum(FC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1, na.rm = TRUE)/sum(FC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1, na.rm = TRUE)/sum(FC_PNC4_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_9) & FC_PNC4_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC4_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
PNC6 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1, na.rm = TRUE)/sum(FC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1, na.rm = TRUE)/sum(FC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1, na.rm = TRUE)/sum(FC_PNC6_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_10) & FC_PNC6_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC6_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
PNC26 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1, na.rm = TRUE)/sum(FC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1, na.rm = TRUE)/sum(FC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1, na.rm = TRUE)/sum(FC_PNC26_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_11) & FC_PNC26_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC26_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
PNC52 <- Inf_Form_Completion_Pnc %>%
  ...
  summarise(
    ...
    "MNH13 Infant Clinical Status" = paste0(
      ...
      " (",
      format(round(sum(!is.na(M13_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1, na.rm = TRUE)/sum(FC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH14 Infant POC" = paste0(
      format(sum(!is.na(M14_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M14_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1, na.rm = TRUE)/sum(FC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "MNH15 Infant Vaccination" = paste0(
      format(sum(!is.na(M15_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(!is.na(M15_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1, na.rm = TRUE)/sum(FC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
  ...
  add_column(
    ...
    "Total" = Inf_Form_Completion_Pnc %>%
      ...
      plyr::summarise(
        ...
        "MNH15 Infant Vaccination" = paste0(
          format(sum(!is.na(M15_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE), nsmall = 0, digits = 2),
          " (",
          format(round(sum(!is.na(M15_VISIT_COMPLETE_12) & FC_PNC52_DENOM == 1 & SITE != "Pakistan", na.rm = TRUE)/sum(FC_PNC52_DENOM==1 & SITE != "Pakistan", na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
          ")")
...
inf_form_comp_pnc <- bind_rows(birthoutcome,
                                 PNC0,
                                 PNC1, 
                                 PNC4, 
                                 PNC6, 
...
inf_form_comp_pnc <- replace_all_sas(inf_form_comp_pnc)

table14_1 <- inf_form_comp_pnc[c(1:9),]
table14_2 <- inf_form_comp_pnc[c(1,10:25),]


## PART 1
prisma_tab14_1 <- tb_theme1(table14_1) %>% 
  tab_header(
    title = md("**PRISMA Table 14** *(continued on next page)*")
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-0 visit, n(%)<sup>a</sup> [Target window: 3 to 5 days]</span>"),
    rows = 2:5
...
## PART 2
prisma_tab14_2 <- tb_theme1(table14_2) %>% 
  tab_header(
    title = md("**PRISMA Table 14** *(continued)*")
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-4 visit, n(%)<sup>c</sup> [Target window: 28 to 35 days]</span>"),
    rows = 2:5
...
```{r}
prisma_tab14_1 <- prisma_tab14_1 %>% gtsave("prisma_tab14_1.png")
knitr::include_graphics("prisma_tab14_1.png")

```

```{r}
prisma_tab14_2 <- prisma_tab14_2 %>% gtsave("prisma_tab14_2.png")
knitr::include_graphics("prisma_tab14_2.png")

```

\newpage

### Figure 7. Heat maps of form completion for infant PNC visits in PRISMA

*Value:* Percent of successful visits during the PNC period.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have a complete visit status (where variable name: `INF_VISIT_MNHXX`=1,
2, or 3) AND passed late window for visit type = i
...
PNC0_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_7 == 1 & HM_PNC0_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC0_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_7==1 & HM_PNC0_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC0_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_7==1 & HM_PNC0_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC0_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC1_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_8 == 1 & HM_PNC1_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC1_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_8==1 & HM_PNC1_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC1_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_8==1 & HM_PNC1_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC1_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC4_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_9 == 1 & HM_PNC4_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC4_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_9==1 & HM_PNC4_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC4_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_9==1 & HM_PNC4_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC4_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC6_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_10 == 1 & HM_PNC6_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_10==1 & HM_PNC6_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_10==1 & HM_PNC6_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC26_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_11 == 1 & HM_PNC26_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_11==1 & HM_PNC26_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_11==1 & HM_PNC26_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC52_PCT <- Inf_Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH13" = paste0(
        format(round(sum(M13_VISIT_COMPLETE_12 == 1 & HM_PNC52_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH14" = paste0(
        format(round(sum(M14_VISIT_COMPLETE_12==1 & HM_PNC52_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),
      
      "MNH15" = paste0(
        format(round(sum(M15_VISIT_COMPLETE_12==1 & HM_PNC52_DENOM_LATE==1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
     ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
inf_heat_map_data_pnc <- bind_rows(PNC0_PCT, PNC1_PCT, PNC4_PCT, PNC6_PCT, PNC26_PCT, PNC52_PCT)


inf_heat_map_data_pnc <- inf_heat_map_data_pnc %>% 
  mutate(PCT_BIN = case_when(FORM %in% c("MNH15") & SITE %in% c("Pakistan") ~ "N/A", TRUE ~ PCT_BIN)
         )
...
UNIQUE_VISITS = as.vector(unique(inf_heat_map_data_pnc$VISIT))

inf_heat_map_data_pnc <- inf_heat_map_data_pnc %>% filter(!is.na(PCT_BIN))
# set levels for percent bins
inf_heat_map_data_pnc$PCT_BIN <- factor(inf_heat_map_data_pnc$PCT_BIN, levels = c("N/A", "0-9", "10-19", "20-29", "30-39", "40-49",
                               "50-59", "60-69", "70-79", "80-89", "90-100"))
# set levels for visits
inf_heat_map_data_pnc$VISIT <- factor(inf_heat_map_data_pnc$VISIT, levels = c("PNC0", "PNC1", "PNC4", "PNC6", "PNC26", "PNC52"))

# inf_heat_map_data_pnc <- inf_heat_map_data_pnc %>%  filter(SITE != "India-SAS")
out <- inf_heat_map_data_pnc %>% 
  mutate(SITE = factor(SITE, levels = rev(levels(factor(SITE))))) %>%  # Reverse the order of levels
  ggplot(aes(x = FORM, y = SITE, fill = PCT_BIN)) +
  geom_tile(color = "grey40", show.legend = TRUE)  + # ,size = 0.2, height = 2, width = 0.9) +  # width = 0.9, 
  facet_grid(rows = vars(VISIT)) +
  scale_fill_manual(values = custom_palette,
                    labels = str_wrap(c("N/A", "0-9%", "10-19%", "20-29%", "30-39%", "40-49%",
                                        "50-59%", "60-69%", "70-79%", "80-89%", "90-100%"), width = 10), 
                    drop = FALSE) +
  labs(fill = str_wrap("Percent Complete", width = 8)) +
  xlab("Expected forms") +
...
### Table 15. Summary of PRISMA infant outcomes.
...
infantoutcomes_summary <- infantoutcomes_summary %>%
  # mutate(`India-SAS` = "-(-)") %>% 
  # relocate(`India-SAS`, .after = `India-CMC`) %>% 
   mutate_at(vars(everything()), ~ifelse(. == "0 (NaN)", "-(-)", .)) %>% 
   mutate_at(vars(everything()), ~ifelse(. == "0 (0)", "-(-)", .)) %>% 
  mutate_at(vars(everything()), ~ifelse(. == "- (-)", "-(-)", .)) 

infantoutcomes_output <- tb_theme1(infantoutcomes_summary) %>% 
  tab_header(
    title = md("**PRISMA Table 15**")
  )  %>% 
...
### Table 16. GSED and FCI in PRISMA/ReMAPP Cohort

*Value:* Are forms completed for each visit? A visit is considered
complete if the data reports the visit was conducted in person or by
phone (variable name: MAT_VISIT_MNHXX ).

*Numerator:* Livebirth with visit type = i (variable name: `TYPE_VISIT`)
AND have complete visit status (where variable name: `MAT_VISIT_MNHXX`=1
or 2) AND passed on time window

*Denominator:* Livebirth passed on time window AND (has not closed out
OR closed out with closeout date \> visit window). *For PNC52 visits
denominator is passed on time window for visit type = i AND has not
closed out OR closed out with reason for closeout=1-year followup
completed).*

```{r}
# zambia_subset_6MO_sf <-  gsed_prisma_data_out %>% filter(SITE == "Zambia") %>% 
#   select(-contains("3MO"), -contains("12MO")) %>% 
#   filter((VC_GSED_SF_6MO_NUM==0 & VC_GSED_6MO_DENOM==1)) %>% 
#   mutate(INF_VISIT_MNH30_6MO = INF_VISIT_MNH30_11,
#          SF_VISIT_DATE_6MO = SF_VISIT_DATE_11
#          ) %>% 
#   select(SITE, MOMID, PREGID, INFANTID, VC_GSED_SF_6MO_NUM, VC_GSED_6MO_DENOM, INF_VISIT_MNH30_6MO, SF_VISIT_DATE_6MO, PASS_6MO)
# 
# zambia_subset_6MO_fci <-  gsed_prisma_data_out %>% filter(SITE == "Zambia") %>% 
#   select(-contains("3MO"), -contains("12MO")) %>% 
#   filter((VC_GSED_FCI_6MO_NUM ==0& VC_GSED_6MO_DENOM==1)) %>% 
#   mutate(INF_VISIT_MNH31_6MO = INF_VISIT_MNH31_11,
#          FCI_VISIT_DATE_6MO = FCI_VISIT_DATE_11,
#          ) %>% 
#   select(SITE, MOMID, PREGID, INFANTID, VC_GSED_FCI_6MO_NUM,
#          VC_GSED_6MO_DENOM_FCI, INF_VISIT_MNH31_6MO, FCI_VISIT_DATE_6MO, PASS_6MO)
# 
# zambia_subset_12MO_sf <-  gsed_prisma_data_out %>% filter(SITE == "Zambia") %>% 
#   select(-contains("3MO"), -contains("6MO")) %>% 
#   filter((VC_GSED_SF_12MO_NUM==0 & VC_GSED_12MO_DENOM==1)) %>% 
#   mutate(INF_VISIT_MNH30_12MO = INF_VISIT_MNH30_12,
#          SF_VISIT_DATE_12MO = SF_VISIT_DATE_12
#          ) %>% 
#   select(SITE, MOMID, PREGID, INFANTID, VC_GSED_SF_12MO_NUM, VC_GSED_12MO_DENOM, INF_VISIT_MNH30_12MO, SF_VISIT_DATE_12MO, ONTIME_12MO) %>% 
#   left_join(mnh24 %>% select(INFANTID, M24_CLOSE_DSSTDAT, M24_CLOSE_DSDECOD), by = c("SITE", "INFANTID"))
# 
# zambia_subset_12MO_fci <-  gsed_prisma_data_out %>% filter(SITE == "Zambia") %>% 
#   select(-contains("3MO"), -contains("6MO")) %>% 
#   filter((VC_GSED_FCI_12MO_NUM ==0& VC_GSED_12MO_DENOM==1)) %>% 
#   mutate(INF_VISIT_MNH31_12MO = INF_VISIT_MNH31_12,
#          FCI_VISIT_DATE_12MO = FCI_VISIT_DATE_12,
#          ) %>% 
#   select(SITE, MOMID, PREGID, INFANTID, VC_GSED_FCI_12MO_NUM,
#          VC_GSED_12MO_DENOM_FCI, INF_VISIT_MNH31_12MO, FCI_VISIT_DATE_12MO, ONTIME_12MO) %>% 
#   left_join(mnh24 %>% select(INFANTID, M24_CLOSE_DSSTDAT, M24_CLOSE_DSDECOD), by = c("SITE", "INFANTID"))
# 
# 
# xl_lst <- list()
# 
# if(exists("zambia_subset_6MO_sf")== TRUE) {
#   xl_lst[['6MO SF missing']] <- zambia_subset_6MO_sf
# }
# 
# if(exists("zambia_subset_12MO_sf")== TRUE) {
#   xl_lst[['12MO SF missing']] <- zambia_subset_6MO_sf
# }
# 
# if(exists("zambia_subset_6MO_fci")== TRUE) {
#   xl_lst[['6MO FCI missing']] <- zambia_subset_6MO_fci
# }
# 
# if(exists("zambia_subset_12MO_fci")== TRUE) {
#   xl_lst[['12MO FCI missing']] <- zambia_subset_6MO_fci
# }
# 
# write.xlsx(xl_lst, file = paste0("D:/Users/stacie.loisate/Documents/Output/Outcomes-Queries/zambia-gsed-2025-08-08", ".xlsx"))


gsed_sf <- gsed_prisma_data_out %>% 
  ...
  summarise(
    ...
    "Denominator (3MO)" = paste0(
      format(sum(VC_GSED_3MO_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2)
    ),
    
    "GSED SF (3MO)" = paste0(
      format(sum(VC_GSED_SF_3MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_SF_3MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_3MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "FCI (3MO)" = paste0(
      format(sum(VC_GSED_FCI_3MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_FCI_3MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_3MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    
    ## 6 MONTHS 
    "Denominator (6MO)" = paste0(
      format(sum(VC_GSED_6MO_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2)
    ),
    
    "GSED SF (6MO)" = paste0(
      format(sum(VC_GSED_SF_6MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_SF_6MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_6MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "FCI (6MO)" = paste0(
      format(sum(VC_GSED_FCI_6MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_FCI_6MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_6MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    ## 12 MONTHS 
    "Denominator (12MO)" = paste0(
      format(sum(VC_GSED_12MO_DENOM == 1, na.rm = TRUE), nsmall = 0, digits = 2)
    ),
    
    "GSED SF (12MO)" = paste0(
      format(sum(VC_GSED_SF_12MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_SF_12MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_12MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")"),
    
    "FCI (12MO)" = paste0(
      format(sum(VC_GSED_FCI_12MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(VC_GSED_FCI_12MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_12MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
      ")")
    
    
  ) %>%
  mutate(`Denominator (3MO)` = case_when(SITE %in% c("India-CMC", "India-SAS", "Kenya","Pakistan", "Zambia") ~ "not assessing", TRUE ~ `Denominator (3MO)`),
         `GSED SF (3MO)` = case_when(SITE %in% c("India-CMC", "India-SAS", "Kenya","Pakistan", "Zambia") ~ "not assessing", TRUE ~ `GSED SF (3MO)`),
         `FCI (3MO)` = case_when(SITE %in% c("India-CMC", "India-SAS", "Kenya","Pakistan", "Zambia") ~ "not assessing", TRUE ~ `FCI (3MO)`),
         `Denominator (6MO)` = case_when(SITE %in% c("Pakistan") ~ "not assessing", TRUE ~ `Denominator (6MO)`),
         `GSED SF (6MO)` = case_when(SITE %in% c("Pakistan") ~ "not assessing", TRUE ~ `GSED SF (6MO)`),
         `FCI (6MO)` = case_when(SITE %in% c("India-CMC","Pakistan") ~ "not assessing", TRUE ~ `FCI (6MO)`),
         `FCI (12MO)` = case_when(SITE %in% c("India-CMC","Pakistan") ~ "not assessing", TRUE ~ `FCI (12MO)`)
         ) %>%
...
gsed_sf_tab_data <- replace_all_sas(gsed_sf_tab_data)

gsed_sf_tab <- tb_theme1(gsed_sf_tab_data) %>%
  tab_header(
    title = md("Table 16. GSED and FCI in PRISMA/ReMAPP Cohort")
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>3 month visit (On time window (+2 weeks): 13 to <17 weeks) (GSED & FCI optional) <sup>a</sup></span>"),
    rows = 1:3
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>6 month visit (On time window (+2 weeks): 26 to <30 weeks) (GSED & FCI optional) <sup>b</sup></span>"),
    rows = 4:6
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>12 month visit (On time window (+2 weeks): 52 to <56 weeks) (GSED required; FCI optional) <sup>c</sup></span>"),
    rows = 7:9
  ) %>%

  row_group_order(groups = c("<span style='font-size: 18px'>3 month visit (On time window (+2 weeks): 13 to <17 weeks) (GSED & FCI optional) <sup>a</sup></span>",
                             "<span style='font-size: 18px'>6 month visit (On time window (+2 weeks): 26 to <30 weeks) (GSED & FCI optional) <sup>b</sup></span>",
                             "<span style='font-size: 18px'>12 month visit (On time window (+2 weeks): 52 to <56 weeks) (GSED required; FCI optional) <sup>c</sup></span>"))  %>%
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>a</sup> denominator is n passed late window with >=17 weeks AND (has not closed out OR closed out with closeout date >=26wks).</span>")
  ) %>% 
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>b</sup> denominator is n passed late window with >=30 weeks AND (has not closed out OR closed out with closeout date >=52wks).</span>")
  ) %>% 
tab_footnote(
  footnote = md("<span style='font-size: 18px'><sup>c</sup> denominator is n passed late window with >=56 weeks OR closed out with closeout reason reported as: *1-year postpartum follow-up period has ended*.</span>")
  ) 

```

```{r, out.width = '100%'}
gsed_sf_tab <- gsed_sf_tab %>% gtsave("gsed_sf_tab.png", expand = 10)
knitr::include_graphics("gsed_sf_tab.png")
```

<!-- ### Table 13. GSED short form sample distribution  -->
...
<!-- *Numerator:* Livebirth with visit type = i (variable name: `TYPE_VISIT`) AND have complete visit status (where variable name: `MAT_VISIT_MNHXX`=1 or 2) AND passed late window  -->

<!-- *Denominator:* Livebirth passed late window AND (has not closed out OR closed out with closeout date \> visit window). *For PNC52 visits denominator is passed late window for visit type = i AND has not closed out OR closed out with reason for closeout=1-year followup completed).* -->

<!-- *Note:* Only includes infants where visit window start date >= PRISMA Expansion (v1.8) start date -->

<!-- ```{r} -->

<!-- gsed_sf <- gsed_sf_data %>%  -->
...
<!--       format(sum(VC_FCI_3MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2), -->

<!--       " (", -->

<!--       format(round(sum(VC_FCI_3MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_3MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2), -->
...
<!--       format(sum(VC_FCI_6MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2), -->

<!--       " (", -->

<!--       format(round(sum(VC_FCI_6MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_6MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2), -->
...
<!--       format(sum(VC_FCI_12MO_NUM==1, na.rm = TRUE), nsmall = 0, digits = 2), -->

<!--       " (", -->

<!--       format(round(sum(VC_FCI_12MO_NUM ==1, na.rm = TRUE)/sum(VC_GSED_12MO_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2), -->
...
<!--     title = md("Table 13. GSED and FCI in PRISMA/ReMAPP Cohort") -->

<!--   ) %>% -->

<!--   tab_row_group( -->

<!--     label = html("<span style='font-size: 18px'>3 month visit (Late window: 13 to <26 weeks) (GSED & FCI optional) <sup>a</sup></span>"), -->
...
<!--     label = html("<span style='font-size: 18px'>12 month visit (Late window: 52 to <64 weeks) (GSED required; FCI optional) <sup>c</sup></span>"), -->

<!--     rows = 7:9 -->

<!--   ) %>% -->

<!--   row_group_order(groups = c("<span style='font-size: 18px'>3 month visit (Late window: 13 to <26 weeks) (GSED & FCI optional) <sup>a</sup></span>", -->

<!--                              "<span style='font-size: 18px'>6 month visit (Late window: 26 to <52 weeks) (GSED & FCI optional) <sup>b</sup></span>", -->

<!--                              "<span style='font-size: 18px'>12 month visit (Late window: 52 to <64 weeks) (GSED required; FCI optional) <sup>c</sup></span>"))  %>% -->

<!--   tab_footnote( -->

<!--     footnote = md("<span style='font-size: 18px'><sup>a</sup> denominator is n passed late window with >=26 weeks gestation AND (has not closed out OR closed out with closeout date >=26wks).</span>") -->

<!--   ) %>%  -->

<!--   tab_footnote( -->

<!--     footnote = md("<span style='font-size: 18px'><sup>b</sup> denominator is n passed late window with >=52 weeks gestation AND (has not closed out OR closed out with closeout date >=52wks).</span>") -->

<!--   ) %>%  -->

<!-- tab_footnote( -->

<!--   footnote = md("<span style='font-size: 18px'><sup>c</sup> denominator is n passed late window with >=64 weeks gestation OR closed out with closeout  -->
...
### Table 17: Bili-ruler (MNH36) Visit and Form Completion

*Value:* Among liveborn infants who were due for an IPC visit after the
Bili-ruler study start date, percent that have Bili-ruler form (MNH36)
completed at four visits (IPC, PNC-0, PNC-1, and PNC-4 OR PNC-6). the
visit is considered `complete` if the data reports the visit was
conducted in person or by phone (variable name: `INF_VISIT_MNH36`=1, 2,
or 3).

*Goal:* 100% completion for all forms.

*Numerator:* By visit type: visit type = i (variable name: `TYPE_VISIT`)
AND have a complete visit status (variable name: \``INF_VISIT_MNHXX`=1,
2, or 3) AND passed late window for visit type = i

*Denominator:* Visit type = i (variable name: `TYPE_VISIT`) AND have a
complete visit status (variable name: `M36_INF_VISIT_MNH36`=1, 2, or 3)
AND passed late window for visit type = i AND DOB+3 (end of IPC window)
is after the Bili-ruler start date. \*Note that if an infant has passed
the late window for IPC, PNC-0, AND/OR PNC-1, PRIOR to the Bili-ruler
study start date, they do NOT count as a part of this denominator.

a.  All infants that should be included in the Bili-ruler substudy
    dataset:

-   Infant is DUE for IPC visit after Bili-ruler study start date (DOB+3
    \> Study start date), OR
-   Infant HAS a COMPLETED IPC visit after Bili-ruler study start date
    (there was a large gap between DOB and IPC visit, and the IPC visit
    occurs after the Bili-ruler start date), OR
-   If IPC visit is missing (both MNH11 and MNH36 at IPC), then PNC0 was
    completed after Bili-ruler start date (IPC visit was possibly
    combined with PNC-0 visit).

b.  Denominator at each visit (IPC, PNC-0, PNC-1, PNC-4/6):

-   All criteria in (a) are met, AND
-   For IPC Visits, Visit was completed after Bili-ruler study start
    date , OR
-   For PNC-X visits, Visit was complete (VC_PNCX_NUM_LATE==1)

```{r}


table17_1 <- c(" ","NA","2024-11-13","2024-12-03","2025-01-13","2024-10-24","2024-11-04") %>%
  t() %>% 
  as.data.frame() %>%
  `colnames<-`(c("Total","Ghana","India-CMC","India-SAS","Kenya","Pakistan","Zambia")) %>% 
  `rownames<-`(c("Bili-ruler Study Start Date")) %>%
  as.data.frame()  

table17_2 <- InfData_BiliRuler %>% 
  ...
  summarise(
    ...
    #OVERALL
    "Total infants expected to be enrolled^a^" = 
      paste0(sum(
        #had a visit
        (M11_VISIT_COMPLETE_6 == 1 & 
           M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
          #due for a visit
          (DOB+3 >= STUDYSTARTDATE |
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))),na.rm=TRUE)),
    
    "**Infants with ANY Bili-ruler visit complete (Target: 900 per site)**" = paste0((sum(
      (!is.na(M36_TYPE_VISIT_6) | !is.na(M36_TYPE_VISIT_7) | !is.na(M36_TYPE_VISIT_8) | !is.na(M36_TYPE_VISIT_9) |
         !is.na(M36_TYPE_VISIT_10) | !is.na(M36_TYPE_VISIT_77))))),
    
    "**Infants with ALL Bili-ruler visits complete (Target: 900 per site)**" = paste0(sum(
      (!is.na(M36_TYPE_VISIT_6) & !is.na(M36_TYPE_VISIT_7) & !is.na(M36_TYPE_VISIT_8) & (!is.na(M36_TYPE_VISIT_9) |!is.na(M36_TYPE_VISIT_10))))),
    
    #IPC
    "**Denominator^b^**" = paste0(sum(
      M11_VISIT_COMPLETE_6 ==1 & 
        (DOB+3 >=  STUDYSTARTDATE | 
        ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
           (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE)))
      ,na.rm=TRUE)),
    
    "MNH36 Form Complete" = paste0(sum(
      (!is.na(M36_TYPE_VISIT_6)) & 
        (M11_VISIT_COMPLETE_6 == 1) &
        (DOB+3 >=  STUDYSTARTDATE | 
        ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
           (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE)))
    ...
    "MNH36 Visit Complete" = paste0(sum(
      M36_INF_VISIT_MNH36_6 %in% c(1,2,3) & 
        M11_VISIT_COMPLETE_6 == 1 & 
        (DOB+3 >=  STUDYSTARTDATE | 
        ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
           (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE)))
    ...
    "**Denominator^b^** " = paste0(sum(
      #Infant had PNC-0 
      (VC_PNC0_NUM_LATE == 1) & 
        #Same IPC denominator
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE)),

    "MNH36 form complete " = paste0(sum(
      !is.na(M36_TYPE_VISIT_7) & 
        (VC_PNC0_NUM_LATE == 1) &
        (DOB+3 >=  STUDYSTARTDATE | 
           ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
              (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE),
    ...
    "MNH36 visit complete " = paste0(sum(
      M36_INF_VISIT_MNH36_7 %in% c(1,2,3) & 
      (VC_PNC0_NUM_LATE == 1) &
        (DOB+3 >=  STUDYSTARTDATE | 
           ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
              (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE),
      " (",
      format(sum(
        #MNH36 visit completed at PNC-0
        M36_INF_VISIT_MNH36_7 %in% c(1,2,3) &
          (VC_PNC0_NUM_LATE == 1) &
          (DOB+3 >=  STUDYSTARTDATE | 
           ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
    ...
    "MNH36 visit complete  " = paste0(sum(
      VC_PNC1_NUM_LATE == 1 & 
        M36_INF_VISIT_MNH36_8 %in% c(1,2,3) &
        M11_VISIT_OBSSTDAT > STUDYSTARTDATE &
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE),
      ...
      * 100,nsmall = 0, digits = 2),
      ")"),
         ...
         #PNC-4 OR PNC-6
    "**Denominator^b^**   " = paste0(sum(
      (VC_PNC4_NUM_LATE == 1 | VC_PNC6_NUM_LATE == 1) & 
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE)),
    
    "MNH36 form complete at either PNC-4 or PNC-6   " = paste0(sum(
      ((VC_PNC4_NUM_LATE == 1 & !is.na(M36_TYPE_VISIT_9)) |
         (VC_PNC6_NUM_LATE == 1 & !is.na(M36_TYPE_VISIT_10))) & 
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE),
      " (",
      format(sum(
        #MNH form completed at PNC-4 or pnc-6
        ((VC_PNC4_NUM_LATE == 1 & !is.na(M36_TYPE_VISIT_9)) |
         (VC_PNC6_NUM_LATE == 1 & !is.na(M36_TYPE_VISIT_10))) & 
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
    ...
    "MNH36 visit complete at either PNC-4 or PNC-6  " = paste0(sum(
        ((VC_PNC4_NUM_LATE == 1  & M36_INF_VISIT_MNH36_9 %in% c(1,2,3)) |  
          (VC_PNC6_NUM_LATE == 1 & M36_INF_VISIT_MNH36_10 %in% c(1,2,3))) & 
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE),
        " (",
    ...
    "MNH36 form completed at PNC-4 AND PNC-6" = paste0(sum(
        (!is.na(M36_TYPE_VISIT_9) & !is.na(M36_TYPE_VISIT_10))  & 
        (VC_PNC4_NUM_LATE == 1 | VC_PNC6_NUM_LATE == 1) & 
        (DOB+3 >=  STUDYSTARTDATE | 
             ((M11_VISIT_OBSSTDAT >= STUDYSTARTDATE) | 
                (is.na(M11_VISIT_OBSSTDAT) & M36_VISIT_OBSSTDAT_6 >= STUDYSTARTDATE))), na.rm=TRUE))
...
table17 <- bind_rows(table17_1,table17_2)

table17_tab <- tb_theme1(table17) %>% 
  tab_header(
    title = md("**Table 17: Bili-ruler Form and Visit Completion**")
  ) %>%
  tab_row_group(
    label=html("<span style='font-size: 18px'>Overall</span>"),
    rows = 1:4
  ) %>%
  tab_row_group(
    label=html("<span style='font-size: 18px'>IPC visit</span>"),
    rows = 5:7
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-0 visit [Target window: 3 to 5 days]</span>"),
    rows = 8:10
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-1 visit [Target window: 7 to 14 days]</span>"),
    rows = 11:13
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>PNC-4 and PNC-6 visit (Referral information at only one of these visits is needed) [Target window: 28-35 days / 6-12 weeks; Referral information only]</span>"),
    rows = 14:17
    ) %>%
  gt::row_group_order(groups = c( "<span style='font-size: 18px'>Overall</span>",
                                  "<span style='font-size: 18px'>IPC visit</span>",
                                  "<span style='font-size: 18px'>PNC-0 visit [Target window: 3 to 5 days]</span>",
                                  "<span style='font-size: 18px'>PNC-1 visit [Target window: 7 to 14 days]</span>",
                                  "<span style='font-size: 18px'>PNC-4 and PNC-6 visit (Referral information at only one of these visits is needed) [Target window: 28-35 days / 6-12 weeks; Referral information only]</span>")) %>%
  tab_style(
    style = list(
      cell_text(weight = "bold")
      ),
...
# table17 <- bind_rows(table17_1,table17_2)
# 
# table17 <- replace_all_sas(table17)
# 
# table17_tab <- tb_theme1(table17) %>% 
#   tab_header(
#     title = md("**Table 17: Bili-ruler Form and Visit Completion**")
...
#     locations = cells_row_groups()) %>% 
#   tab_footnote(
#     footnote = html("<span style='font-size: 18px'><sup>a</sup> Infant is DUE for IPC visit after Bili-ruler study start date (DOB+3 > Study start date) OR Infant HAS a COMPLETED IPC visit after Bili-ruler study start date (there was a large gap between DOB and IPC visit, and the IPC visit occurs after the Bili-ruler start date) OR If IPC visit is missing (both MNH11 and MNH36 at IPC), then PNC0 was completed after Bili-ruler start date (IPC visit was possibly combined with PNC-0 visit).</span>")) %>% 
#     tab_footnote(
#     footnote = html("<span style='font-size: 18px'><sup>b</sup> All criteria in (a) are met AND, For IPC Visits, Visit was completed after Bili-ruler study start date OR For PNC-X visits, Visit was complete (VC_PNCX_NUM_LATE==1).</span>")) 

```

```{r}

table17_tab <- table17_tab %>% gtsave("table17_tab.png", expand = 10)
knitr::include_graphics("table17_tab.png")
```
...
### Table 18: Protocol compliance for ReMAPP fatigue assessment
...
remapp_table1 <- replace_all_sas(remapp_table1)
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>Note:</sup> Pakistan stopped FACIT on 2025-01-06. Data is not expected after this date.</span>")
)
...
### Table 19: Participant eligibility for the ReMAPP healthy cohort
...
remapp_table2 <- healthyOutcome %>% 
  ...
  summarise("Total enrolled in PRISMA since ReMAPP launch" = paste0("(n=", n(), ")"),
            "Eligible for healthy cohort (regardless of GA at enrollment), n(%) ^a^" = paste0(sum(HEALTHY_ELIGIBLE_14WK == 1, na.rm=TRUE),
                                        " (", format(sum(HEALTHY_ELIGIBLE_14WK == 1, na.rm=TRUE)/sum(ENROLL == 1)*100, digits = 2, nsmall=0), ")"),
              ## new row 
            "Eligible for healthy cohort (enrolled <14 weeks), n(%)" = paste0(sum(HEALTHY_ELIGIBLE == 1, na.rm=TRUE),
            ...
            "Pending, n(%)^b^" = paste0(sum(HEALTHY_ELIGIBLE == 55, na.rm=TRUE),
                                " (", format(sum(HEALTHY_ELIGIBLE == 55, na.rm=TRUE)/sum(ENROLL == 1)*100, digits = 2, nsmall=0), ")")
  ) %>% 
    t() %>% as.data.frame() %>% 
  `colnames<-`(c(.[1,])) %>% 
  slice(-1) %>% 
  ...
  plyr::summarise("Total enrolled in PRISMA since ReMAPP launch" = paste0("(n=", sum(ENROLL==1), ")"), 
            "Eligible for healthy cohort (regardless of GA at enrollment) ^a^, n(%)" = paste0(sum(HEALTHY_ELIGIBLE_14WK == 1, na.rm=TRUE),
                                        " (", format(sum(HEALTHY_ELIGIBLE_14WK == 1, na.rm=TRUE)/sum(ENROLL == 1)*100, digits = 2, nsmall=0), ")"),
              ## new row 
            "Eligible for healthy cohort (enrolled <14 weeks), n(%)" = paste0(sum(HEALTHY_ELIGIBLE == 1, na.rm=TRUE),
                           " (", format(sum(HEALTHY_ELIGIBLE == 1, na.rm=TRUE)/sum(ENROLL == 1)*100, digits = 2, nsmall=0), ")"),
...
remapp_table2 <- replace_all_sas(remapp_table2)
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> a participant is considered eligible regardless of gestational age at enrollment (GA at enrollment <14 weeks criteria excluded).</span>")
) %>%
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> a participant is considered pending if default values 55 or -5 are reported for any of the criteria. Participants who are pending are otherwise eligible, but are missing/pending confirmation.</span>")
)
...
### Figure 8: Eligibility status and missingness by ReMAPP healthy cohort criteria

*Value:* Illustrates the distribution of each healthy cohort criteria
(C1-C20) among women enrolled in PRISMA since ReMAPP launch. Missing
...
### Table 20: Protocol compliance for ReMAPP hemoglobin measurements by CBC
...
*Numerator:* CBC peformed at visit type = i AND (variable name:
`TYPE_VISIT`) AND have a complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i

*Denominator:* Completed MNH08 with visit type = i OR passed late window
for visit type = i AND (has not delivered OR delivered with gestational
age \> late visit window) AND (has not closed out OR closed out with
closeout date \> visit window)
  ...
  group_by(SITE) %>%
    summarise(
          "Total enrolled since ReMAPP launch" = paste0(
            ...
            "Enrollment" = paste0(sum(HB_COMPLETED_1 == 1 & DenHBV1 == 1, na.rm = TRUE),
                                        " (", format(sum(HB_COMPLETED_1 == 1 & DenHBV1 == 1, na.rm = TRUE)/sum(DenHBV1 == 1, na.rm = TRUE)*100, digits = 2, nsmall=0), ")"),
            "ANC20" = paste0(sum(HB_COMPLETED_2 == 1 & DenHBV2 == 1, na.rm = TRUE),
                                        " (", format(sum(HB_COMPLETED_2 == 1 & DenHBV2 == 1, na.rm = TRUE)/sum(DenHBV2 == 1, na.rm = TRUE)*100, digits = 2, nsmall=0), ")"),
            "ANC28" = paste0(sum(HB_COMPLETED_3 == 1 & DenHBV3 == 1, na.rm = TRUE),
                                        " (", format(sum(HB_COMPLETED_3 == 1 & DenHBV3 == 1, na.rm = TRUE)/sum(DenHBV3 == 1, na.rm = TRUE)*100, digits = 2, nsmall=0), ")"),
  
            "ANC36" = paste0(sum(HB_COMPLETED_5 == 1 & DenHBV5 == 1, na.rm = TRUE),
                                        " (", format(sum(HB_COMPLETED_5 == 1 & DenHBV5 == 1, na.rm = TRUE)/sum(DenHBV5 == 1, na.rm = TRUE)*100, digits = 2, nsmall=0), ")"),
...
remapp_table3 <- replace_all_sas(remapp_table3)
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>a</sup> denominator is n completed MNH08 with visit type=Enroll OR passed late window with >19 weeks gestation AND (has not delivered or delivered with gestational age > late visit window) AND (has not closed out or closed out with closeout date > late visit window).</span>")
) %>% 
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n completed MNH08 with visit type=ANC-20 OR missed late window with >25 weeks gestation AND (has not delivered or delivered with gestational age > late visit window) AND (has not closed out or closed out with closeout date > late visit window).</span>")
) %>% 
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>c</sup> denominator is n completed  MNH08 with visit type=ANC-28 OR missed late window with >30 weeks gestation AND (has not delivered or delivered with gestational age > late visit window) AND (has not closed out or closed out with closeout date > late visit window).</span>")
) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>d</sup> denominator is n completed  MNH08 with visit type=ANC-36 OR missed late window with >38 weeks gestation AND (has not delivered or delivered with gestational age > late visit window) AND (has not closed out or closed out with closeout date > late visit window).</span>")
) %>% 
  tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>e</sup> denominator is n completed  MNH08 with visit type=PNC-6 OR missed late window with >12 weeks postpartum AND (has not closed out or closed out with closeout date > late visit window).</span>")
) # %>% 
#   tab_footnote(
#     footnote = html("<span style='font-size: 18px'><sup>Note:</sup> all denominators include participants who withdrew, were lost to follow up or investigator terminated participation. Only 1-year follow up completed, 42-day follow up completed, and maternal death are excluded.</span>")
# ) 
...
### Table 21. ReMIND Study Status

*Value:* Distribution of birth outcomes and ReMIND enrollment status
among babies born to ReMAPP Aim 3 mothers. Enrollment is defined as
livebirth and mother consented to ReMIND.

```{r}

## REMIND TABLE
gsed_aim3_tab_summary <- gsed_aim3_data %>%
  filter(!is.na(SITE)) %>% 
  ## If India-SAS doesn't have data, add empty row here
  # mutate(existing_sas = ifelse(SITE == "India-SAS", 1, 0)) %>%
  # # ## Add empty rows for missing SITE values if the specific site doesn't exist
  ...
  summarise(
    ...
    ## ReMAPP Aim 3 
    "ReMAPP Aim 3 moms, n" =  paste0(
      format(sum(MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE), nsmall = 0, digits = 2)),
    
    "Delivered, n (%)" =  paste0(
      format(sum(PREG_OUTCOME %in% c("Livebirth", "Fetal loss") & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME %in% c("Livebirth", "Fetal loss") & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)/sum(MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
    "Not yet delivered, n (%)" =  paste0(
      format(sum(PREG_OUTCOME == "Participant not yet delivered" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME == "Participant not yet delivered" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)/sum(MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
     "Closed out before delivery, n (%)" =  paste0(
      format(sum(PREG_OUTCOME == "Closed out before pregnancy outcome could be reported" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME == "Closed out before pregnancy outcome could be reported" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)/sum(MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
      "Pending pregnancy outcome, n (%)" =  paste0(
      format(sum(PREG_OUTCOME == "Missing pregnancy outcome" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME == "Missing pregnancy outcome" & MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)/sum(MOM_REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
      
    ## Infants born to ReMAPP Aim 3 moms
    "Infants delivered, n" =  paste0(
      format(sum(PREG_OUTCOME %in% c("Livebirth", "Fetal loss"), na.rm = TRUE), nsmall = 0, digits = 2)),
    
     "Livebirths, n (%)" =  paste0(
      format(sum(PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME =="Livebirth", na.rm = TRUE)/sum(PREG_OUTCOME %in% c("Livebirth", "Fetal loss"), na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
      "Fetal losses, n (%)" =  paste0(
      format(sum(PREG_OUTCOME =="Fetal loss", na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(PREG_OUTCOME =="Fetal loss", na.rm = TRUE)/sum(PREG_OUTCOME %in% c("Livebirth", "Fetal loss"), na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
    ## Infants enrollment status
    "Livebirths, n" =  paste0(
      format(sum(PREG_OUTCOME == "Livebirth", na.rm = TRUE), nsmall = 0, digits = 2)),

    "Enrolled, n" =  paste0(
      format(sum(REMIND_ENROLLMENT_STATUS ==1 & PREG_OUTCOME == "Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(REMIND_ENROLLMENT_STATUS ==1 & PREG_OUTCOME == "Livebirth", na.rm = TRUE)/sum(PREG_OUTCOME == "Livebirth", na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
    "Not enrolled, n" =  paste0(
      format(sum(REMIND_ENROLLMENT_STATUS ==0 & PREG_OUTCOME == "Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(REMIND_ENROLLMENT_STATUS ==0 & PREG_OUTCOME == "Livebirth", na.rm = TRUE)/sum(PREG_OUTCOME == "Livebirth", na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")"),
    
      "Pending, n" =  paste0(
      format(sum(REMIND_ENROLLMENT_STATUS ==55 & PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
      " (",
      format(round(sum(REMIND_ENROLLMENT_STATUS ==55 & PREG_OUTCOME =="Livebirth", na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100), nsmall = 0, digits = 2),
      ")")
  ) %>%
...
# gsed_aim3_tab_summary[c(12), c("India-CMC", "Zambia")] = "n/a (-)"

```

```{r}

gsed_aim3_tab_summary_labels <- gsed_aim3_tab_summary %>%
  mutate_at(vars(everything()), ~ifelse(. == "0 (NaN)", "0 (0)", .))

gsed_aim3_tab_summary_labels <- replace_all_sas(gsed_aim3_tab_summary_labels)

gsed_aim3_summary_tab <- tb_theme1(gsed_aim3_tab_summary_labels) %>%
  tab_header(
    title = md("Table 21. ReMIND Study Status")
  ) %>%

  tab_row_group(
    label = html("<span style='font-size: 18px'>ReMAPP Aim 3<sup>a</sup></span>"),
    rows = 1:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>Infants born to ReMAPP Aim 3 moms<sup>b</sup></span>"),
    rows = 6:8
  ) %>%
tab_row_group(
    label = html("<span style='font-size: 18px'>Infant ReMIND enrollment status<sup>c</sup></span>"),
    rows = 9:12
  ) %>%
  row_group_order(groups = c("<span style='font-size: 18px'>ReMAPP Aim 3<sup>a</sup></span>",
                             "<span style='font-size: 18px'>Infants born to ReMAPP Aim 3 moms<sup>b</sup></span>",
                             "<span style='font-size: 18px'>Infant ReMIND enrollment status<sup>c</sup></span>"
                             ))  %>%
tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>a</sup> denominator is n ReMAPP aim 3 moms.</span>")
) %>%
tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n infants born to ReMAPP aim 3 moms.</span>")
) %>%
  tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>c</sup> denominator is n livebirths.</span>")
) %>%
  tab_footnote(
  footnote = html("<span style='font-size: 18px'>Note: Pending pregnancy outcome indicates the EDD has passed but no birth outcome has been reported.</span>")
)

```

```{r, out.width = '100%'}
gsed_aim3_summary_tab <- gsed_aim3_summary_tab %>% gtsave("gsed_aim3_summary_tab.png", expand = 10)
knitr::include_graphics("gsed_aim3_summary_tab.png")
```

\newpage

### Table 22. ReMIND Assessment Completion
...
*Numerator:* Aim 3 enrolled livebirth with visit date within visit
window AND have complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed on time window. Enrollment is
defined as livebirth and mother consented to ReMIND.

*Denominator:* Aim 3 enrolled livebirth passed on time window AND (has
not closed out OR closed out with closeout date \> visit window). *For
PNC52 visits denominator is passed on time window for visit type = i AND
has not closed out OR closed out with reason for closeout=1-year
followup completed).*
...
gsed_aim3_tab <- gsed_aim3_data %>%
  ...
  summarise(
    ...
    ## OVERALL 
    "Enrolled in ReMIND, n" =  paste0(
      format(sum(REMIND_ENROLLMENT_STATUS ==1 & PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2)),
      
    #  "Livebirths, n" =  paste0(
    #   format(sum(PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(PREG_OUTCOME =="Livebirth", na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
    # "Enrolled in ReMIND, n" =  paste0(
    #   format(sum(REMIND_ENROLLMENT_STATUS ==1, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(REMIND_ENROLLMENT_STATUS ==1, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
    # "Not enrolled in ReMIND, n" =  paste0(
    #   format(sum(REMIND_ENROLLMENT_STATUS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(REMIND_ENROLLMENT_STATUS ==0, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
    #   "Pending ReMIND enrollment status, n" =  paste0(
    #   format(sum(REMIND_ENROLLMENT_STATUS ==55 & PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(REMIND_ENROLLMENT_STATUS ==55 & PREG_OUTCOME =="Livebirth", na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
    ## 3 MONTHS
    "Denominator (3MO)" = paste0(
  ...
  ## OVERALL 
    ...
    #   format(sum(REMIND_ENROLLMENT_STATUS ==1 & PREG_OUTCOME =="Livebirth", na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(REMIND_ENROLLMENT_STATUS ==1 & PREG_OUTCOME =="Livebirth", na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
    #     "Not enrolled in ReMIND, n" =  paste0(
    #   format(sum(REMIND_ENROLLMENT_STATUS ==0, na.rm = TRUE), nsmall = 0, digits = 2),
    ...
    # 
    #   "Pending ReMIND enrollment status, n" =  paste0(
    #   format(sum(REMIND_ENROLLMENT_STATUS ==55, na.rm = TRUE), nsmall = 0, digits = 2),
    #   " (",
    #   format(round(sum(REMIND_ENROLLMENT_STATUS ==55, na.rm = TRUE)/sum(REMAPP_AIM3_ENROLL==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
    #   ")"),
    # 
        ## 3 MONTHS
        "Denominator (3MO)" = paste0(
...
gsed_aim3_tab_data <- replace_all_sas(gsed_aim3_tab_data)

gsed_aim3_tab <- tb_theme1(gsed_aim3_tab_data) %>%
  tab_header(
    title = md("Table 22. ReMIND Assessment Completion")
  ) %>%

  tab_row_group(
    label = html("<span style='font-size: 18px'>3 month visit (On time window: 13 to <=15 weeks) (GSED required; FCI optional) <sup>b</sup></span>"),
    rows = 2:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>6 month visit (On time window: 26 to <=28 weeks) (GSED & FCI optional) <sup>c</sup></span>"),
    rows = 6:9
  ) %>%
tab_row_group(
    label = html("<span style='font-size: 18px'>12 month visit (On time window: 52 to <=54 weeks) (GSED required; FCI optional) <sup>d</sup></span>"),
    rows = 10:13
  ) %>%
    row_group_order(groups = c(NA,
                             "<span style='font-size: 18px'>3 month visit (On time window: 13 to <=15 weeks) (GSED required; FCI optional) <sup>b</sup></span>",
                             "<span style='font-size: 18px'>6 month visit (On time window: 26 to <=28 weeks) (GSED & FCI optional) <sup>c</sup></span>",
                             "<span style='font-size: 18px'>12 month visit (On time window: 52 to <=54 weeks) (GSED required; FCI optional) <sup>d</sup></span>"
                             ))  %>%
tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>a</sup> denominator is n infants born to ReMAPP Aim 3 mothers. Pending status includes cases where consent is conducted at the 3 month visit, mother has not yet delivered, etc.</span>")
) %>%
  tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n passed late window with >15 weeks AND (has not closed out OR closed out with closeout date >=26wks).</span>")
) %>%
  tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>c</sup> denominator is n passed late window with >28 weeks AND (has not closed out OR closed out with closeout date >=52wks).</span>")
) %>%
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>d</sup> denominator is n passed late window with >54 weeks OR closed out with closeout
  reason reported as: *1-year postpartum follow-up period has ended*.</span>")
  )
...
```{r, out.width = '100%'}
gsed_aim3_tab <- gsed_aim3_tab %>% gtsave("gsed_aim3_tab.png", expand = 10)
knitr::include_graphics("gsed_aim3_tab.png")
```
