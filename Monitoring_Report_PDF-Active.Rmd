---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2025-06-17)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
# load(paste0("D:/Users/stacie.loisate/Box/Monitoring-Report-Active/data/df_criteria.RData"))
...
**Included data last updated in Synapse:** 2025-06-13
...
*Value:* Among liveborn infants with PNC visits, are forms completed for each visit? A visit is considered
complete if the data reports the visit was conducted in person or by
phone (variable name: INF_VISIT_MNHXX ). If there is no late window for
a visit, the on-time window is used.
...
*Denominator:* Passed late window for visit type = i AND
(has not closed out OR closed out with closeout date \> visit window) *For PNC52 visits denominator is passed late window for visit type = i
AND has not closed out OR closed out with reason for closeout=1-year
followup completed).*
...
*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND
have a complete visit status (where variable name: `INF_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i
...
*Denominator:* Livebirth passed on time window AND (has not closed out OR
closed out with closeout date \> visit window). *For PNC52 visits
denominator is passed on time window for visit type = i AND has not closed
out OR closed out with reason for closeout=1-year followup completed).*
...
*Value:* Among liveborn infants who were due for an IPC visit after the Bili-ruler study start date,  percent that have Bili-ruler form (MNH36) completed at four visits (IPC, PNC-0, PNC-1, and PNC-4 OR PNC-6).  the visit is considered `complete` if the data reports the visit was conducted in person or by phone (variable name: `INF_VISIT_MNH36`=1, 2, or 3).

*Goal:* 100% completion for all forms.

*Numerator:* By visit type: visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (variable name: \``INF_VISIT_MNHXX`=1, 2, or 3) AND passed late window for visit type = i

*Denominator:* Visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (variable name: `M36_INF_VISIT_MNH36`=1, 2, or 3) AND passed late window for visit type = i
AND DOB+3 (end of IPC window) is after the Bili-ruler start date.
*Note that if an infant has passed the late window for IPC, PNC-0, AND/OR PNC-1, PRIOR to the Bili-ruler study start date, they do NOT count as a part of this denominator.  

```{r}

table17_1 <- c(" ","2024-11-13","2024-12-03","2025-01-13","2024-10-24","2024-11-04") %>%
  t() %>% 
  as.data.frame() %>%
...
table17_2 <- InfData_BiliRuler %>% 
  ...
  summarise(
    ...
    "**MNH36 form complete at IPC, PNC-0, or PNC-1 (goal: 900 per site)**" = paste0((sum(
    ...
    "**ALL MNH36 complete at IPC, PNC-0, PNC-0, and [PNC-4 OR PNC-6] (goal: 900 per site)**" = paste0(sum(
...
*Value:* Percent of fatigue assessments (MNH26) completed among women expected for visits in the ANC and PNC periods, regardless of visit completion status. A visit is considered `complete` if the data reports the visit was conducted in person or by phone (variable name: `MAT_VISIT_MNHXX`). The table below shows protocol compliance distributions for MNH26 forms following site launch of ReMAPP. These forms can be filled out either during individual visits (e.g., ANC20) or during combined visits after the gestational age window has passed (e.g., combined Enrollment and ANC20 visits).For example, the denominator for MNH26 completed at ANC32 is the number of participants who have passed the ANC36 late window to allow for combined visits.
...
*Numerator:* CBC peformed at visit type = i AND (variable name: `TYPE_VISIT`)
AND have a complete visit status (where variable name:
`MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i

*Denominator:* Completed MNH08 with visit type = i OR passed late window for visit type = i AND (has not
delivered OR delivered with gestational age \> late visit window) AND
(has not closed out OR closed out with closeout date \> visit window)
...
*Value:* Distribution of birth outcomes and ReMIND enrollment status among babies born to ReMAPP Aim 3 mothers. Enrollment is defined as livebirth and mother consented to ReMIND. 
...
*Numerator:* Aim 3 enrolled livebirth with visit date within visit window AND have complete visit status (where variable name: `MAT_VISIT_MNHXX`=1 or 2) AND passed on time window. Enrollment is defined as livebirth and mother consented to ReMIND. 

*Denominator:* Aim 3 enrolled livebirth passed on time window AND (has not
closed out OR closed out with closeout date \> visit window). *For PNC52
visits denominator is passed on time window for visit type = i AND has not
closed out OR closed out with reason for closeout=1-year followup
completed).*
...
gsed_aim3_tab <- tb_theme1(gsed_aim3_tab_data) %>%
  ...
  tab_row_group(
    label = html("<span style='font-size: 18px'>3 month visit (On time window (+2 weeks): 13 to <17 weeks) (GSED required; FCI optional) <sup>b</sup></span>"),
    rows = 2:5
  ) %>%
  tab_row_group(
    label = html("<span style='font-size: 18px'>6 month visit (On time window (+2 weeks): 26 to <30 weeks) (GSED & FCI optional) <sup>c</sup></span>"),
    rows = 6:9
  ) %>%
tab_row_group(
    label = html("<span style='font-size: 18px'>12 month visit (On time window (+2 weeks): 52 to <56 weeks) (GSED required; FCI optional) <sup>d</sup></span>"),
    rows = 10:13
  ) %>%
  row_group_order(groups = c(NA,
                             "<span style='font-size: 18px'>3 month visit (On time window (+2 weeks): 13 to <17 weeks) (GSED required; FCI optional) <sup>b</sup></span>",
                             "<span style='font-size: 18px'>6 month visit (On time window (+2 weeks): 26 to <30 weeks) (GSED & FCI optional) <sup>c</sup></span>",
                             "<span style='font-size: 18px'>12 month visit (On time window (+2 weeks): 52 to <56 weeks) (GSED required; FCI optional) <sup>d</sup></span>"
                             ))  %>%
tab_footnote(
...
tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n passed late window with >=17 weeks AND (has not closed out OR closed out with closeout date >=26wks).</span>")
) %>%
  tab_footnote(
  footnote = html("<span style='font-size: 18px'><sup>c</sup> denominator is n passed late window with >=30 weeks AND (has not closed out OR closed out with closeout date >=52wks).</span>")
) %>%
  tab_footnote(
    footnote = md("<span style='font-size: 18px'><sup>d</sup> denominator is n passed late window with >=56 weeks OR closed out with closeout
