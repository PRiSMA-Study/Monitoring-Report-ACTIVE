...
<!-- *Value:* Among participants who have passed their IPC window and not have any IPC forms. Distributions of gestational age at the time of last visit is shown below. -->
...
<!-- #   filter(GESTAGE_AT_VISIT_DAYS >= 300 & GESTAGE_AT_VISIT_DAYS<400) -->

<!-- out <- MatData_Ipc_Visits_Mat %>% -->
<!--   # only include participants who are enrolled and who do not have a pregnancy end (ENDPREG_DAYS == NA) -->
<!--   filter(GA42_MISSING_IPC==1) %>% -->
<!--   select(SITE, MOMID, PREGID, INFANTID, contains("GESTAGE_AT_VISIT_DAYS")) %>% -->
<!--   # pivot longer to collapse all GA at visits into a single column called "GESTAGE_AT_VISIT_DAYS" -->
<!--   pivot_longer(cols = -c(SITE, MOMID, PREGID, INFANTID), -->
<!--                values_to = "GESTAGE_AT_VISIT_DAYS") %>% -->
...
<!--   summarise(AGE_LAST_SEEN = max(GESTAGE_AT_VISIT_DAYS, na.rm = TRUE))  -->

<!-- ggplot(data = out, aes(x = AGE_LAST_SEEN)) + -->
<!--   geom_histogram() + -->
...
<!--   xlab("Gestational Age At Latest Visit (weeks)") + -->
<!--   ylab("Count") -->
