---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2023-10-03)"
output:
  html_document:
    df_print: paged
    toc: yes
...
##### Kenya : 2023-09-15 {.unlisted .unnumbered}
##### Pakistan : 2023-09-15 {.unlisted .unnumbered}
##### Ghana : 2023-09-15 {.unlisted .unnumbered}
##### Zambia : 2023-09-15 {.unlisted .unnumbered}
##### India CMC : 2023-09-15 {.unlisted .unnumbered}
...
tb_prescreen_enroll <- MatData_Screen_Enroll %>% 
  filter(
    ...
    SITE == "India-CMC" & (M00_SCRN_OBSSTDAT_1 > date_last_enrll_cmc)  
  ) %>% 
  group_by(SITE) %>% 
...
enroll_cum_PA %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("25-09-2022"), UploadDate)) + ## UPDATE EACH RUN
  xlab("Enrollment Week") +
...
enroll_cum_KE %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("20-11-2022"), UploadDate)) +    
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Kenya") 
...
enroll_cum_GA %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("26-12-2022"), UploadDate)) +  
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Ghana")
...
enroll_cum_ZA %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("15-12-2022"), UploadDate)) + 
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Zambia") 
...
enroll_cum_CMC %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("18-07-2023"), UploadDate)) + 
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("India-CMC") 
...
VISIT_COMPLETE_PNC <- Visit_Complete_Pnc %>% 
      ...
      "PNC = 52" = paste0(
        format(sum(VC_PNC26_NUM == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
        format(round(sum(VC_PNC26_NUM == 1, na.rm = TRUE)/sum(VC_PNC52_DENOM==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
        ")"),
...
VISIT_COMPLETE_PNC <- Visit_Complete_Pnc %>%
      ...
      "PNC = 52" = paste0(
        format(sum(VC_PNC26_NUM_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
        format(round(sum(VC_PNC26_NUM_LATE == 1, na.rm = TRUE)/sum(VC_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2),
        ")"),
...
#### `r emo::ji("pushpin")` Table 8: PNC Protocol Compliance
...
#### `r emo::ji("pushpin")` Table 9: PNC Form Completion
*Value: Are forms completed regardless of visit status? Goal: 100% completion for all forms, except MNH08 where lab results are expected to lag.*
<br>
...
<!-- #### `r emo::ji("pushpin")` Table 9: Infant PNC Form Completion -->
<!-- *Value: Are forms completed regardless of visit status?* -->
<!-- <br> -->
<!-- <br> -->

<!-- *Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND have any visit status (variable name: `INF_VISIT_MNHXX`) AND passed window for visit type = i -->
<!-- <br> -->
<!-- *Denominator:* Any form with visit type = i (variable name: `TYPE_VISIT`) AND have any visit status (variable name: `INF_VISIT_MNHXX`) AND passed window for visit type = i -->

<!-- ```{r infants FC} -->
<!-- ## ANC visits - ultrasound form -->
<!-- # denominator: BIRTH_OUTCOME_YN = n who have a birth outcome -->
<!-- # Participants are included in the numerator of this table if, for each form, they have visit type = i AND have any visit status AND passed window for visit type = 1. Participants are included Visit status is defined by MAT_VISIT_MNHxx variables in each form.* -->
<!-- infats <- MatData_Pnc_Visits %>% -->
<!--   group_by(SITE) %>% -->
<!--   summarise( -->
<!--   "total birth outcomes" = paste0( -->
<!--     "(n=",sum(BIRTH_OUTCOME_YN==1), ")" -->
<!--   ) -->
<!--   ) %>% ungroup() %>% -->
<!--   t() %>% -->
<!--   as.data.frame() %>% -->
...
<!--                )  %>% -->
<!--   mutate_all(funs(str_replace(., "NaN", "0"))) %>% -->
<!--   kbl(caption = "", longtable = TRUE, booktabs = TRUE) %>% -->
<!--     footnote(alphabet = c("denominator is n completed any form with any visit status AND visit type=PNC0 AND passed on-time window with >5 days following birth", -->
<!--                       "denominator is n completed any form with any visit status AND visit type=PNC1 AND passed on-time window with 14 days following birth", -->
<!--                       "denominator is n completed any form with any visit status AND visit type=PNC4 AND passed on-time window with >35 days following birth", -->
<!--                       "denominator is n completed any form with any visit status AND visit type=PNC6 AND passed on-time window with >7 wks following birth", -->
<!--                       "denominator is n completed any form with any visit status AND visit type=PNC26 AND passed on-time window with >28 wks following birth", -->
<!--                       "denominator is n completed any form with any visit status AND visit type=PNC52 AND passed on-time window with >54 wks following birth"), -->
<!--              general = "Passed window denominators for all visits are calculated by date and time of birth reported in MNH09. If an date of birth is missing, the particpant is removed from this table.", -->
<!--              number = "n is any infant with a live birth defined by `INF_DSTERM` in MNH11", -->
<!--              footnote_order = c("number", "alphabet", "general")) %>% -->
<!--   kable_paper(bootstrap_options = "striped", -->
<!--               full_width = T, html_font = "Cambria", position = "left", -->
<!--               latex_options = c("repeat_header", "HOLD_position")) %>% -->
<!--   row_spec(0, extra_css = "border-bottom: 0px white;") %>% -->
<!--   row_spec(0:1, extra_css = "padding: 0.1px") %>% -->
<!--   row_spec(c(2,6,10,14,18,22 ), bold = TRUE) %>% -->
<!--   pack_rows(paste0("PNC = 0, n(%)^a^ \nTarget window: 3 to 5 days"),2, 5, -->
<!--             label_row_css = "color: steelblue;") %>% -->
<!--   pack_rows("PNC = 1, n(%)^b^ \nTarget window: 7 to 14 days", 6, 9, -->
<!--             label_row_css = "color: steelblue;") %>% -->
<!--   pack_rows("PNC = 4, n(%)^c^ \nTarget window: 28 to 35 days", 10, 13, -->
<!--             label_row_css = "color: steelblue;") %>% -->
<!--   pack_rows("PNC = 6, n(%)^d^ \nTarget window: 6 to 7 wks", 14, 17, -->
<!--             label_row_css = "color: steelblue;") %>% -->
<!--   pack_rows("PNC = 26, n(%)^e^ \nTarget window: 26 to 28 wks", 18, 21, -->
<!--             label_row_css = "color: steelblue;") %>% -->
<!--   pack_rows("PNC = 52, n(%)^f^ \nTarget window: 52 to 54 wks", 22, 25, -->
<!--             label_row_css = "color: steelblue;") -->
