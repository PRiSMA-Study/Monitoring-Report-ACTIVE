...
```{r setup, include=FALSE}
  # Last Updated: May 01, 2023
      # Combined tables 1a and 2a to include only 1 table that provides a snapshot of prescreening/enrollment in the most recent 1 week. 
      # renamed the remaining tables to account for changes in tables 1-2
...
##### Kenya : 2023-04-28 {.unlisted .unnumbered}
##### Pakistan : 2023-04-28 {.unlisted .unnumbered}
##### Ghana : 2023-04-28 {.unlisted .unnumbered}
##### Zambia : 2023-04-28 {.unlisted .unnumbered}
...
table(MatData_Pnc_Visits$M26_VISIT_COMPLETE_10)
...
date_last_enrll_ga <- lastenroll %>% filter(SITE == "Ghana") %>% select(last_enroll) %>%  unique() %>% mutate(last_enroll = last_enroll-14) %>% pull()
date_last_enrll_za <- lastenroll %>% filter(SITE == "Zambia") %>% select(last_enroll) %>%  unique() %>% mutate(last_enroll = last_enroll-7) %>% pull()

```
##### **Last Enrollment Date: ** 
##### Kenya : 2023-04-28 {.unlisted .unnumbered}
##### Pakistan : 2023-04-19 {.unlisted .unnumbered}
##### Ghana : 2023-04-27 {.unlisted .unnumbered}
##### Zambia : 2023-03-30 {.unlisted .unnumbered}
...
#### `r emo::ji("pushpin")` Table 1: Pre-screening and Enrollment numbers for PRISMA MNH Study for the most recent <span style="text-decoration:underline">one week</span> 
...
#### `r emo::ji("pushpin")` Table 2: <span style="text-decoration:underline">Cumulative</span>  pre-screening numbers for PRISMA MNH Study 
...
#### `r emo::ji("pushpin")` Table 3: <span style="text-decoration:underline">Cumulative</span> enrollment numbers for PRISMA MNH Study 
...
enroll_cum %>% 
  ...
  geom_point(shape = 15) +
  geom_line() +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("25-09-2022"), dmy("28-04-2023"))) + ## UPDATE EACH RUN 
  theme_bw() +
...
enroll_cum_PA %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("25-09-2022"), dmy("19-04-2023"))) + ## UPDATE EACH RUN
  xlab("Enrollment Week") +
...
enroll_cum_KE %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("20-11-2022"), dmy("28-04-2023"))) +     ## UPDATE EACH RUN
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Kenya") 
...
enroll_cum_GA %>%
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("26-12-2022"), dmy("27-04-2023"))) +     ## UPDATE EACH RUN
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Ghana")
...
enroll_cum_ZA <- MatData_Screen_Enroll %>% 
  ...
  filter(SCREEN == 1 & SITE == "Zambia" & !is.na(START_DATE_MOM_WK), 
         START_DATE_MOM_WK > dmy("25/12/2022")) %>% 
  group_by(START_DATE_MOM_WK) %>% 
  summarise(enroll_n=sum(ENROLL==1),
            expect_n=mean(EXPECT)) %>% 
  mutate(
...
enroll_cum_ZA %>% 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1),
        legend.position="bottom",
        panel.grid.minor = element_blank()) +
  scale_x_date(date_breaks = "1 week", date_labels = ("%m-%d"), limits = c(dmy("25-12-2022"), dmy("30-03-2023"))) + ## UPDATE EACH RUN
  xlab("Enrollment Week") +
  ylab("Site Participant Expected and Enrolled") +
  ggtitle("Zambia") 
...
#### `r emo::ji("pushpin")` Table 5: ANC study visit & procedure completion metrics
