---
title: "<span style='font-size: 18px'> <span style='text-align: center'> PRISMA & ReMAPP Monitoring Report (Issued: 2024-04-09)"
output:
  pdf_document:
    toc: no
    toc_depth: 4
...
**PRISMA MNH:** Pregnancy Risk, Infant Surveillance, and Measurement Alliance (PRISMA) Maternal and Newborn Health (MNH) Study: A multi-site, prospective, open cohort study of pregnancy and postpartum health outcomes

&nbsp;
&nbsp;

**ReMAPP:** Redefining Maternal Anemia in Pregnancy and Postpartum (ReMAPP) Study: A multisite, international, population-based cohort study to establish global hemoglobin thresholds for maternal anemia
...
**Included data last updated in Synapse:** 2024-04-05 (Zambia: 2024-03-08)
...
#### **Last enrollment date: **
  ...
  + Ghana : 2024-03-07
  + Zambia : 2024-03-01
  + India CMC : 2024-03-22
  + India SAS : 2024-03-28
...
*Value:* Provides a snapshot of weekly pre-screening and enrollment progress.
...
geom_text(aes(label = n_ineligible, x = SITE, y = n_pct+0.25), 
  ...
  coord_flip() +  
  scale_fill_manual(values = c("#b40e20", "#e58600", "#e2d300", "#47acc8", "#01a08a","#0c775e"), #
...
tb_enroll <- MatData_Screen_Enroll %>%
  ...
  summarise("Screened, n" = sum(SCREEN == 1, na.rm=TRUE),
    ...
    ")"),
            ...
            "Gestational age >=20 weeks" = paste0(sum(GA20 == 0, na.rm=TRUE),
...
### STACKED BAR CHART
total_bysite <- MatData_Screen_Enroll %>% 
  filter(SCREEN == 1 & SCREEN_DENOM == 1 & ENROLL==0) %>% 
  select(SITE, AGE, GA20, CATCHAREA, CATCHREMAIN, CONSENT) %>%
  rename( "Did not meet age requirement" = AGE,
          "Gestational age >=20 weeks" = GA20,
...
total_inelg = MatData_Screen_Enroll %>% 
  ...
  select(SITE, ENROLL) %>% 
  group_by(SITE) %>% 
  summarise(n_ineligible = sum(ENROLL==0, na.rm = TRUE)) 
...
desired_order2 <- unique(factor(barchart_data_out$name,
                          levels = c( "Did not meet age requirement",
...
geom_text(aes(label = n_ineligible, x = SITE, y = n_pct+0.25), 
  ...
  coord_flip() +  
  scale_fill_manual(values = c("#b40e20", "#e58600", "#e2d300", "#47acc8", "#01a08a","#0c775e"), #
                     labels = c("Did not meet age requirement","Gestational age >=20 weeks",
                                  "Outside catchment area", "Doesn't plan to remain in catchment area", "Did not consent"),
                     breaks=  c("Did not meet age requirement","Gestational age >=20 weeks",
                                  "Outside catchment area", "Doesn't plan to remain in catchment area", "Did not consent"),
                     name = NULL,
...
*Value:* Among women who are expected for a visit, is the visit complete. A visit is considered completed if the data reports the visit was conducted in person or by phone (variable name: `MAT_VISIT_MNHXX`).
...
table5 <- bind_rows(VISIT_COMPLETE_ANC,
          VISIT_COMPLETE_PNC) %>% 
    `rownames<-` (c(## anc (1-10)
                 "Enrollment (None)", ## NONE - SKIP WINDOW
                 "Denominator, n",
                 "ANC-20 (23 to 25wks)",
                 "Denominator, n ",
                 "ANC-28 (None)", ## NONE - SKIP WINDOW
                 "Denominator, n  ",
                 "ANC-32 (31 to 33wks)", ## use on time windows
                 "Denominator, n   ",
                 "ANC-36 (38wks to delivery)", ## use on time windows
                 "Denominator, n    ",
                 ## pnc (11-22)
                 "PNC-0 (None)", ## NONE - SKIP WINDOW
                 "Denominator, n     ",
                 "PNC-1 (None)", ## NONE - SKIP WINDOW
                 "Denominator, n      ",
                 "PNC-4 (None)", ## NONE - SKIP WINDOW
                 "Denominator, n       ",
                 "PNC-6 (8 to 12wks)",
                 "Denominator, n        ",
                 "PNC-26 (29 to 39wks)",
...
prisma_tab5_1 <- tb_theme1(table5_anc) %>% 
  ...
  tab_style(
      ...
      locations = cells_stub(rows = c('Enrollment (None)',
                 "ANC-20 (23 to 25wks)",
                 "ANC-28 (None)", ## NONE - SKIP WINDOW
                 "ANC-32 (31 to 33wks)", ## use on time windows
                 "ANC-36 (38wks to delivery)")) ## use on time windows
...
*Value:* Among women successfully contacted during the PNC period, percent who had all expected visit forms completed. A visit is considered `complete` if the data reports the visit was conducted in person or by phone (variable name: `MAT_VISIT_MNHXX`).
...
PNC0 <- Form_Completion_Pnc %>%
    ...
    summarise(
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_7) & PNC0_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_7) & PNC0_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
PNC1 <- Form_Completion_Pnc %>%
    ...
    summarise(
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_8) & PNC1_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_8) & PNC1_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
PNC4 <- Form_Completion_Pnc %>%
    ...
    summarise(
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_9) & PNC4_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_9) & PNC4_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
PNC6 <- Form_Completion_Pnc %>%
    ...
    summarise(
      ...
      "MNH05 Maternal Anthropometrics" = paste0(
        format(sum(!is.na(M05_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH07 Maternal Specimen Collection" = paste0(
        format(sum(!is.na(M07_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH08 Maternal Lab" = paste0(
        format(sum(!is.na(M08_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH25 Maternal Depression" = paste0(
        format(sum(!is.na(M25_VISIT_COMPLETE_10) & PNC6_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
PNC26 <- Form_Completion_Pnc %>%
    ...
    summarise(
      ...
      "MNH05 Maternal Anthropometrics" = paste0(
        format(sum(!is.na(M05_VISIT_COMPLETE_11) & PNC26_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_11) & PNC26_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_11) & PNC26_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
PNC52 <- Form_Completion_Pnc %>%
    ...
    summarise(
     ...
     "MNH05 Anthropometrics" = paste0(
        format(sum(!is.na(M05_VISIT_COMPLETE_12) & PNC52_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH06 Maternal POC" = paste0(
        format(sum(!is.na(M06_VISIT_COMPLETE_12) & PNC52_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
      ...
      "MNH12 Maternal PNC Clinical Status" = paste0(
        format(sum(!is.na(M12_VISIT_COMPLETE_12) & PNC52_PASS_LATE == 1, na.rm = TRUE), nsmall = 0, digits = 2),
        " (",
...
tab_footnote(
    footnote = html("<span style='font-size: 18px'><sup>b</sup> denominator is n passed ANC-36 late window with >42 weeks gestation AND (has not delivered OR delivered with gestational age >32wks) AND (has not closed out OR closed out with closeout date >32wks).</span>")
) %>% 
tab_footnote(
...
### Figure 5a. Heat maps of form completion for ANC visits in PRISMA 
*Value:* Percent of successful visits during the ANC period.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (where variable name: `MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i

*Denominator:* Passed late window for visit type = i AND (has not delivered OR delivered with gestational age > late visit window) AND (has not closed out OR closed out with closeout date > visit window) OR has any form with visit type = i
...
ENROLLMENT_PCT <- Heat_Maps_Anc %>% 
  ...
  summarise(
    ...
    "MNH01" = paste0(
      format(round(sum(M01_VISIT_COMPLETE_1 == 1 , na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
      2), nsmall = 0, digits = 2)),

    "MNH02" = paste0(
      format(round(sum(ENROLL == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
      2), nsmall = 0, digits = 2)),

    "MNH03" = paste0(
      format(round(sum(M03_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
                   2), nsmall = 0, digits = 2)),
    
    "MNH04" = paste0(
      format(round(sum(M04_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
                   2), nsmall = 0, digits = 2)),
    
    "MNH05" = paste0(
      format(round(sum(M05_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
                   2), nsmall = 0, digits = 2)),
    
    "MNH06" = paste0(
      format(round(sum(M06_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
                   2), nsmall = 0, digits = 2)),
    
    "MNH07" = paste0(
      format(round(sum(M07_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100, 
                   2), nsmall = 0, digits = 2)),
    
    "MNH08" = paste0(
      format(round(sum(M08_VISIT_COMPLETE_1 == 1, na.rm = TRUE)/sum(HM_ENROLL_DENOM_LATE==1, na.rm = TRUE)*100,
                   2), nsmall = 0, digits = 2))
) %>% 
...
ANC20_PCT <- Heat_Maps_Anc %>% 
  ...
  summarise(
    ...
    "MNH04" = paste0(
      format(round(sum(M04_VISIT_COMPLETE_2 == 1  , na.rm = TRUE)/sum( HM_ANC20_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH05" = paste0(
      format(round(sum(M05_VISIT_COMPLETE_2 == 1  , na.rm = TRUE)/sum( HM_ANC20_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH06" = paste0(
      format(round(sum(M06_VISIT_COMPLETE_2 == 1  , na.rm = TRUE)/sum( HM_ANC20_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH07" = paste0(
      format(round(sum(M07_VISIT_COMPLETE_2 == 1  , na.rm = TRUE)/sum( HM_ANC20_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH08" = paste0(
      format(round(sum(M08_VISIT_COMPLETE_2 == 1  , na.rm = TRUE)/sum( HM_ANC20_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2))
  ) %>%
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
ANC28_PCT <- Heat_Maps_Anc %>% 
  ...
  summarise(
    ...
    "MNH04" = paste0(
      format(round(sum(M04_VISIT_COMPLETE_3 == 1, na.rm = TRUE)/sum(HM_ANC28_DENOM_LATE==1, na.rm = TRUE)*100, 2), 
             nsmall = 0, digits = 2)),
    
    "MNH05" = paste0(
      format(round(sum(M05_VISIT_COMPLETE_3 == 1, na.rm = TRUE)/sum(HM_ANC28_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH06" = paste0(
      format(round(sum(M06_VISIT_COMPLETE_3 == 1, na.rm = TRUE)/sum(HM_ANC28_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH07" = paste0(
      format(round(sum(M07_VISIT_COMPLETE_3 == 1, na.rm = TRUE)/sum(HM_ANC28_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH08" = paste0(
      format(round(sum(M08_VISIT_COMPLETE_3 == 1, na.rm = TRUE)/sum(HM_ANC28_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2))) %>%
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
               values_to = "COMPLETE_PCT") %>% 
...
ANC32_PCT <- Heat_Maps_Anc %>% 
  ...
  summarise(
    ...
    "MNH01" = paste0(
      format(round(sum(M01_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH04" = paste0(
      format(round(sum(M04_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH05" = paste0(
      format(round(sum(M05_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH06" = paste0(
      format(round(sum(M06_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH07" = paste0(
      format(round(sum(M07_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH08" = paste0(
      format(round(sum(M08_VISIT_COMPLETE_4 == 1, na.rm = TRUE)/sum(HM_ANC32_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2))
  ) %>%
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
ANC36_PCT <- Heat_Maps_Anc %>% 
  ...
  summarise(
    ...
    "MNH04" = paste0(
      format(round(sum(M04_VISIT_COMPLETE_5 == 1, na.rm = TRUE)/sum(HM_ANC36_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH05" = paste0(
      format(round(sum(M05_VISIT_COMPLETE_5 == 1, na.rm = TRUE)/sum(HM_ANC36_DENOM_LATE==1, na.rm = TRUE)*100, 2), 
             nsmall = 0, digits = 2)),
    
    "MNH06" = paste0(
      format(round(sum(M06_VISIT_COMPLETE_5 == 1, na.rm = TRUE)/sum(HM_ANC36_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH07" = paste0(
      format(round(sum(M07_VISIT_COMPLETE_5 == 1, na.rm = TRUE)/sum(HM_ANC36_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2)),
    
    "MNH08" = paste0(
      format(round(sum(M08_VISIT_COMPLETE_5 == 1, na.rm = TRUE)/sum(HM_ANC36_DENOM_LATE==1, na.rm = TRUE)*100, 2),
             nsmall = 0, digits = 2))) %>%
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
               values_to = "COMPLETE_PCT") %>% 
...
out2  <- heat_map_data_anc %>%
  ...
  xlab("Expected forms") +
  ylab("Study site") + 
  ggtitle(paste0("Form completion at ", tolower(UNIQUE_VISITS[2]), " visit (target window: 23 to 25wks)"))  +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme_minimal() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(size = 8),
...
### Figure 5b. Heat maps of form completion for PNC visits in PRISMA 
*Value:* Percent of successful visits during the PNC period.

*Numerator:* By form: visit type = i (variable name: `TYPE_VISIT`) AND have a complete visit status (where variable name: `MAT_VISIT_MNHXX`=1 or 2) AND passed late window for visit type = i

*Denominator:* Passed late window for visit type = i AND (has not closed out OR closed out with closeout date > visit window) OR has any form with visit type = i
...
PNC0_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_7 == 1, na.rm = TRUE)/sum(HM_PNC0_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_7==1, na.rm = TRUE)/sum(HM_PNC0_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC1_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_8 == 1, na.rm = TRUE)/sum(HM_PNC1_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_8 == 1, na.rm = TRUE)/sum(HM_PNC1_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC4_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_9 == 1, na.rm = TRUE)/sum(HM_PNC4_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_9 == 1, na.rm = TRUE)/sum(HM_PNC4_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC6_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH05" = paste0(
        format(round(sum(M05_VISIT_COMPLETE_10 == 1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_10 == 1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH07" = paste0(
        format(round(sum(M07_VISIT_COMPLETE_10 == 1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH08" = paste0(
        format(round(sum(M08_VISIT_COMPLETE_10 == 1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_10 == 1, na.rm = TRUE)/sum(HM_PNC6_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC26_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
      ...
      "MNH05" = paste0(
        format(round(sum(M05_VISIT_COMPLETE_11 == 1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2), nsmall = 0, digits = 2)),

      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_11 == 1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH07" = paste0(
        format(round(sum(M07_VISIT_COMPLETE_11 == 1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH08" = paste0(
        format(round(sum(M08_VISIT_COMPLETE_11 == 1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_11 == 1, na.rm = TRUE)/sum(HM_PNC26_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
      ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
PNC52_PCT <- Heat_Maps_Pnc %>%
    ...
    summarise(
     ...
     "MNH05" = paste0(
        format(round(sum(M05_VISIT_COMPLETE_12 == 1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),


      "MNH06" = paste0(
        format(round(sum(M06_VISIT_COMPLETE_12 == 1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2)),

      "MNH12" = paste0(
        format(round(sum(M12_VISIT_COMPLETE_12 == 1, na.rm = TRUE)/sum(HM_PNC52_DENOM_LATE==1, na.rm = TRUE)*100, 2),
               nsmall = 0, digits = 2))
     ) %>% 
  pivot_longer(cols = -SITE,
               names_to = "FORM", 
...
out3  <- heat_map_data_pnc %>% 
  ...
  xlab("Expected forms") +
  ylab("Study site") +
  ggtitle(paste0("Form completion at ", tolower(UNIQUE_VISITS[3]), " visit (target window: 28 to 25 days)"))  +
  guides(fill = guide_legend(reverse = TRUE)) +
  theme_minimal() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(size = 8),
...
### Table 1: Protocol compliance for ReMAPP fatigue assessment
...
### Table 2: Participant eligibility for the ReMAPP healthy cohort
...
### Figure 1: Eligibility status and missingness by ReMAPP healthy cohort criteria 
*Value:* Illustrates the distribution of each healthy cohort criteria (C1-C19) among women enrolled in PRISMA since ReMAPP launch. Missing means missing form, note that it includes pending data (test not performed yet or result pending).
...
* C6: Not iron deficient (serum ferritin >15 mcg/L– adjusted for inflammation) 
* C7: No subclinical inflammation (CRP>=5 mg/L and/or AGP>=1 g/L) 
* C8: Systolic blood pressure <140 mmHg and diastolic blood pressure <90 mmHg 
* C9: No previous reported low birth weight delivery
* C10: No previous reported stillbirth
* C11: No previous reported unplanned cesarean delivery
* C12: No hemoglobinopathies: SS, SC, SE, EE, CC, SD-Punjab, Sβthal, Eβthal, Cβthal, CD-Punjab, ED-Punjab, D-D-Punjab, D-Punjabβthal, Thalassemia major, Thalassemia intermedia, glucose-6-phosphate dehydrogenase deficiency, or Alpha thalassemia
* C13: No reported cigarette smoking, tobacco chewing, or betel nut use during pregnancy
* C14: No reported alcohol consumption during pregnancy
* C15: No known history or current chronic disease including cancer, kidney disease, and cardiac conditions
* C16: No known history or current HIV
* C17: No current malaria infection (per rapid diagnostic test)
* C18: No current Hepatitis B virus infection (per rapid diagnostic test)
* C19: No current Hepatitis C virus infection (per rapid diagnostic test)

\newpage


### Table 3: Protocol compliance for ReMAPP hemoglobin measurements by CBC  
*Value:* Protocol compliance for CBC hemoglobin measurements at required ReMAPP study visits, regardless of visit completion status. 
  ...
  group_by(SITE) %>%
    summarise(
          "Total enrolled since ReMAPP launch" = paste0(
            ...
            "PNC6" = paste0(sum(HB_COMPLETED_10 == 1, na.rm = TRUE),
...
### Figure 2: Plot of PRISMA hemoglobin measurements using any method by gestational age
