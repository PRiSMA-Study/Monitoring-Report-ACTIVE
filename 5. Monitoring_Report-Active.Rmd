...
desired_order2 <- unique(factor(barchart_data_out$name,
                          levels = c("Did not meet age requirement",
                                     "Outside catchment area",
                                     "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
                                     "Doesn't plan to remain in catchment area",
                                     "Did not consent")))

# desired_order2 <- unique(factor(barchart_data_out$name,
#                           levels = c("Did not consent",
#                                      "Did not meet age requirement",
#                                      "Doesn't plan to remain in catchment area",
#                                      
#                                      "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
...
ggplot(barchart_data_out %>% mutate(SITE = factor(SITE, levels = desired_order)), 
  ...
  geom_text(aes(label = n_ineligible, x = SITE, y = n_pct+0.25), 
    ...
    coord_flip() +  
    # scale_fill_manual(values = c("#b40e20","#e2d300", "#e58600",  "#47acc8", "#01a08a","#0c775e"), 
    #                   labels = c("Did not meet age requirement","Outside catchment area",
    #                               "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
    #                               "Doesn't plan to remain in catchment area", "Did not consent"),
    #                    breaks =  c("Did not meet age requirement","Outside catchment area",
    #                               "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
    #                               "Doesn't plan to remain in catchment area", "Did not consent"),
    #                    name = NULL,
    #                    guide = guide_legend(nrow = 3)) +
      scale_fill_manual(values = c("#b40e20","#e2d300", "#e58600",  "#47acc8", "#01a08a","#0c775e"), #
                       labels = str_wrap(c("Did not meet age requirement", 
                                   "Outside catchment area",
                                   "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
                                   "Doesn't plan to remain in catchment area",
                                   "Did not consent"), width = 40), # Adjust the width as needed
                        breaks =  c("Did not meet age requirement", 
                                   "Outside catchment area",
                                   "Non-viable intrauterine pregnancy >=20 weeks gestation (confirmed via ultrasound)",
                                   "Doesn't plan to remain in catchment area",
                                   "Did not consent"),
...
table5 <- bind_rows(VISIT_COMPLETE_ANC,
    ...
    `rownames<-` (c(## anc (1-10)
                 ...
                 "Denominator, n   ",
                 "ANC-36 (38 wks to delivery)", ## use on time windows
                 "Denominator, n    ",
                 ## pnc (11-22)
